    ...
    {   
		...
		private void CrossSignalLineEvent()
		{
			if (_isSignalLineHigher)
			{
				...
				if (SignalBottomToTopOrder.Equals(AdvisorOrderType.Buy)) Sell(SignalType.SignalLineDownTop); 		
				if (SignalBottomToTopOrder.Equals(AdvisorOrderType.Sell)) Buy(SignalType.SignalLineDownTop);	
			}
			else
			{
				...
				if (SignalTopToBottomOrder.Equals(AdvisorOrderType.Buy)) Sell(SignalType.SignalLineTopDown);			
				if (SignalTopToBottomOrder.Equals(AdvisorOrderType.Sell)) Buy(SignalType.SignalLineTopDown);	
			}
		}
		...
		private void CrossZeroLineEvent()
		{
			if (_isSignalWasPositive) 
			{
				...
				if (ZeroLineBottomToTopOrder.Equals(AdvisorOrderType.Buy)) Sell(SignalType.ZeroLineDownTop);			
				if (ZeroLineBottomToTopOrder.Equals(AdvisorOrderType.Sell)) Buy(SignalType.ZeroLineDownTop);	
			}
			else 
			{
                Print("LR_indicator_and_MACD_up_to_down_intersection");
				...
				if (ZeroLineTopToBottomOrder.Equals(AdvisorOrderType.Buy)) Sell(SignalType.ZeroLineTopDown);			
				if (ZeroLineTopToBottomOrder.Equals(AdvisorOrderType.Sell)) Buy(SignalType.ZeroLineTopDown);	
			}
		}
