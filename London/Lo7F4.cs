...
namespace IPro.TradeSystems
{
    ...
    public class ZZ_Ex1 : TradeSystem
    {
		...
		[Parameter("Пауза :", DefaultValue = 20, MinValue = 2, MaxValue = 200)]
		public int kf { get;set; }	
		
		[Parameter("Пауза Stop :", DefaultValue = 5, MinValue = 2, MaxValue = 200)]
		...
		public double Fs,Fs1,min1,max1,max2,min2;
		private bool F15U,F15D,torg;
		private static double dl = 0.00030;
...
Print("{0} -  FsU={4} FsD={5} F15U={6} --  tmM15={1} Fs={2} Fs1={3} ",DTime,tmM15,Fs,Fs1,FsU,FsD,F15U);			
			if ( DTime.Hour==7 ) {	
				...
				case 0:
						{
							...
							max1=MAX;
							if(max1>max2) { MAX=max2; max1=max2; }
							return Math.Round(MAX+dl+Instrument.Spread, Instrument.PriceScale);
						}
				case 1:
						{
							...
							min1=MIN;
							if(min1<min2) { MIN=min2; min1=min2; }
							return Math.Round(MIN-dl-Instrument.Spread, Instrument.PriceScale);
						}
				default: 
					break;
			}
