...
{
    [TradeSystem("Lo7FF")]          //copy of "Lo7F"
    public class ZZ_Ex1 : TradeSystem
    {
		...
		[Parameter("Fractal K :", DefaultValue = 5, MinValue = 0, MaxValue = 200)]
		...
		public double Fs2,Fs1,min1,max1,max2,min2,dlt,frSU,frSD,frSU1,frSD1;
...
if (Instrument.Name == "GBPJPY") {  mgB=118; mgS=218; }
        ...
        protected override void NewQuote()
        {
				if (_lastIndexM15 < _barM15.Range.To-1) {     		    	
					Fs2=Fs1;// Запоминаем пред значение фишера
					Fs1=_ftoIndM15.FisherSeries[_barM15.Range.To-1]; // Серия фишера 
					if(_lastIndexM15>0 && Fs1>Fs2) F15U=true; else F15U=false;// Определяем тренд
...
//========= Запоминаем значения последних фракталов =========================================================			
			if (_frInd.TopSeries[Bars.Range.To-fr]>0)    frSU=_frInd.TopSeries[Bars.Range.To-fr];
			if (_frInd.BottomSeries[Bars.Range.To-fr]>0) frSD=_frInd.BottomSeries[Bars.Range.To-fr];
...
//====== Открытие позиций =============================================================================================================
			if ( DTime.Hour==7 ) {	
					if (posGuidBuy==Guid.Empty && !F15U && FsU && torg) { torg=false;
					...
					if (posGuidSell==Guid.Empty && F15U && FsD && torg) { torg=false;	
...
//==== Трелинг позиции ===========================================================================================================================   
		...
		{		
		  if(posGuidBuy!=Guid.Empty && frSU1!=frSU)  { frSU1=frSU; var tr = Trade.UpdateMarketPosition(posGuidBuy,	  getSL(1),null," - update TP,SL"); 
//		  Print("{0} BUY  UPDATE FsU={2} FsD={3} min1={4} min2={5} posGuidBuy={1}",Bars[ci].Time,posGuidBuy.ToString() ,FsU,FsD,min1,min2);
		  }
		  if(posGuidSell!=Guid.Empty && frSD1!=frSD) { frSD1=frSD;  var tr = Trade.UpdateMarketPosition(posGuidSell,  getSL(0),null," - update TP,SL"); 
...
//=====  Определяем стоп ============================
		protected double getSL(int type)
		{
			...
			{
				case 0:
						{return Math.Round(frSU+dlt+Instrument.Spread, Instrument.PriceScale);}
				case 1:
						{return Math.Round(frSD-dlt-Instrument.Spread, Instrument.PriceScale);}
				default: 
					break;
			}
			return 0.0;
