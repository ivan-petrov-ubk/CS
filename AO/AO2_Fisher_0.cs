...
{
    [TradeSystem("AO2_Fisher_0")]   //copy of "AO2_Eliote3Wave"
    public class AO : TradeSystem
    {
        ...
        protected override void NewBar()
        {
			...
			sF  = _ftoInd.FisherSeries[Bars.Range.To-1];  // Фішеер - Плюс-зверху Мінус - знизу
	 		sF1 = _ftoInd.FisherSeries[Bars.Range.To-5];
			
			//  Пересечение Фишер		
            if (sF1<0 && sF>0 && aoUp2<=0 && aoUp1>=0)        vlR.Time=Bars[Bars.Range.To-1].Time; // ВВЕРХ
			if (sF1>0 && sF<0 && aoDown2>=0 && aoDown1<=0)    vlB.Time=Bars[Bars.Range.To-1].Time; // ВНИЗ
