...
// 1) Дожидаемся закрытие дневной свечи !
// 2) Проверяем основные параметры:
//    размер дневной свечи по EURUSD должен быть больше 40 пунктов (для 4-х значного Брокера) — расстояние от максимума до минимума свечи (хвостов)
//    минимальный размер ТЕЛА свечи должен быть >0 для пары EURUSD (для других валютных пар тело должно быть еще больше)
// 3) Если параметры закрытой дневной свечи нас удовлетворяют, то:
// Если свеча Медвежья — то в этот день устанавливаем только ордер Бай-стоп!
using System;
...
{
    [TradeSystem("Trend Reversal2")] //copy of "Trend Reversal"
