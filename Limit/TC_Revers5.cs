...
{
    [TradeSystem("TC_Revers5")] //copy of "TC_Revers4"
    public class TC_Revers4 : TradeSystem
    {
		...
		public double SL_Buy,SL_Sell,SL=0.0003;
		public double TP=0.0002;
				          ...
				          frUpL=Bars[Bars.Range.To-5].Low;   
			  			  if(frUpL>aGuba5 && frUpL>aChelust5 && frUpL>aZub5) //Низ Бар-Фрактала выше Alligator 
								...
								if (posGuidBuy!=Guid.Empty && Trade.GetPosition(posGuidBuy).State==PositionState.Pending)
				   				{   var result = Trade.UpdatePendingPosition(posGuidBuy, 0.1, frUpH+TP, frDownL-TP,null); 
				       				if (result.IsSuccessful) { posGuidBuy = result.Position.Id;
 			           				Print("06 UpdatePending - !появился новый фр-рабочий - переносим цену Open={0} SL={1} TP={2} - {3}",fr_all_Up,SL_Sell,fr_all_Down-TP,DTime);}
				   				}
 							...
 							if (posGuidBuy!=Guid.Empty && Trade.GetPosition(posGuidBuy).State==PositionState.Active) 
				   				{  var result2=Trade.UpdateMarketPosition(posGuidBuy,frDownL-TP,null, null); 
				   				   if (result2.IsSuccessful) {  posGuidBuy = result2.Position.Id;
									    Print("14 SELL - !UpdateActive StopLoss когда Появился новый фрактал Up - SL={0} - {1}",frUpH,DTime);}  
			       				}
			  					...
			  					if(frDownH<aGuba5 && frDownH<aChelust5 && frDownH<aZub5) 
									...
									//======= UPDATE BUY  =========================================================================	
									   if (posGuidSell!=Guid.Empty && Trade.GetPosition(posGuidSell).State==PositionState.Pending) 
										{ var result = Trade.UpdatePendingPosition(posGuidSell, 0.1, frDownL-TP,frUpH+TP, null); 
											if (result.IsSuccessful) { posGuidSell = result.Position.Id;  
				   							Print("10 Sell UpdatePending - появился новый фр-рабочий - переносим цену и StopLoss - Open={0} - SL={1} - TP={2} ",fr_all_Down,SL_Buy,fr_all_Up+TP,DTime);
										    } else { Print("10 Sell UpdatePending - ERROR - - Open={0} - SL={1} - TP={2}  ",fr_all_Down,SL_Buy,fr_all_Up+TP,DTime); }
										}	
					 				   Print("02 Фрактал НИЖЕ Алигатора! - {0}",Bars[Bars.Range.To-1].Time); } else 
									{// Если же фр-касается аллигатора - отменяем пред отложеній ордер
										if (posGuidSell!=Guid.Empty && Trade.GetPosition(posGuidSell).State==PositionState.Pending) 
			       							{   var res = Trade.CancelPendingPosition(posGuidSell);
			     	   							if (res.IsSuccessful) { posGuidSell = Guid.Empty;
				       							Print("06-2 появился новый фр-рабочий ВНИЗ и касается АЛЛ отменяем отложеный Sell CancelPending - {0}",DTime); }
				   							}
									}
									 // Появился новый фрактал ВНИЗ и ОТКРЫТА позиция ВВЕРХУ Buy - переносим стоп - Работает!
				   					if (posGuidSell!=Guid.Empty && Trade.GetPosition(posGuidSell).State==PositionState.Active) 
				     					  ...
				     					  if (result2.IsSuccessful) {  posGuidSell = result2.Position.Id;
											  Print("14 BuyStop - !UpdatePending StopLoss когда Появился новый фрактал Down - SL={0} {1}",frDownL-TP,DTime);}  
			                            }	
			  				}
...
//====  BUYLIMIT  START  =====================================================================================================
			...
			{
				        // Открываем BuyLimit
				var result = Trade.OpenPendingPosition(Instrument.Id, ExecutionRule.SellStop, 0.1, fr_all_Down-TP, 0, Stops.InPips(200,200), null, null, null);
						 if (result.IsSuccessful) { posGuidSell=result.Position.Id;  
						         Print("03 SellStop! OPEN - O={0} - SL={1} - TP={2} {3}",fr_all_Down,SL_Buy,fr_all_Up+TP,DTime);
													} else {
						         //var result2 = Trade.Sell(Instrument.Id, 0.1);
							     //if (result2.IsSuccessful) { posGuidSell = result2.Position.Id;
							     Print("04 SellStop! ERROR - O={0} - SL={1} - TP={2} {3}",fr_all_Down,SL_Buy,fr_all_Up+TP,DTime);  	 
					           }  
			}
//==   SELLIMIT START   ================================================================================						  
			if(frUp && fr_all_Up_L && posGuidBuy==Guid.Empty)
			{ 
			var result = Trade.OpenPendingPosition(Instrument.Id, ExecutionRule.BuyStop, 0.1, fr_all_Up+TP, 0, Stops.InPips(200,200), null, null, null);
				//Stops.InPips(200,null)
				if (result.IsSuccessful) { posGuidBuy = result.Position.Id;
					   Print("05 SellLimit OPEN - O={0} - SL={1} - TP={2} {3}",fr_all_Up,SL_Sell,fr_all_Down-TP,DTime);
				}  else  {   
						//var result2=Trade.Sell(Instrument.Id, 0.1);
						//if (result2.IsSuccessful) { posGuidBuy = result2.Position.Id; 
					  Print("06 SellLimit ERROR - O={0} - SL={1} - TP={2} {3}",fr_all_Up,SL_Sell,fr_all_Down-TP,DTime);     
					}
			}		
