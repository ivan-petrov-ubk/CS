			...
			if (posGuidSell!=Guid.Empty && Trade.GetPosition(posGuidSell).State==PositionState.Pending) {
			 ...
			 // 2. После 18:00 выставляем 2 ордера: Buy Limit и Sell Limit на расстоянии Х пунктов от текущей цены.
			  ...
			  if(xp>400) {
			   ...
			   var result = Trade.OpenPendingPosition(Instrument.Id, ExecutionRule.BuyStop, 0.1, highestPrice-0.0005, 0, Stops.InPips(250,210), null, null, null);
			 ...
			 if(Bars[Bars.Range.To-24].Open<Bars[Bars.Range.To-1].Close) //Вверх
			   ...
			   var result1 = Trade.OpenPendingPosition(Instrument.Id, ExecutionRule.SellStop, 0.1,  lowestPrice+0.0005, 0, Stops.InPips(250,210), null, null, null);
