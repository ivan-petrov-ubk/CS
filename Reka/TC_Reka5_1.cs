...
// Сигнал №4  Buy - Цена касается 20-й MA.
			...
			if(i>7 &&  i1==1 && Bars[Bars.Range.To-1].Close<SMa20 )  
				 ...
				 { Buy(); pBuy=true; i1=0; i3=0; i2=0;    Print("Buy!! - {0} = {1} ",(SMa50-SMa20),Bars[Bars.Range.To-1].Time);} 
			...
			if(j>7 && j1==1 &&  (Bars[Bars.Range.To-1].Close)>SMa20 ) 
				 ...
				 { Sell(); pSell=true;   j1=0; j3=0; j2=0;  j=0; Print("SEL!! - {0} = {1}",(SMa50-SMa20),Bars[Bars.Range.To-1].Time);}
        ...
        if (pBuy && i3>5 && _positionGuid!=Guid.Empty) { ClosePosition(); pBuy=false; i3=0; } 
		...
		if (pSell && j3>5 && _positionGuid!=Guid.Empty) { ClosePosition(); pSell=false; j3=0;}		
