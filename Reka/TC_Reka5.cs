...
namespace IPro.TradeSystems
{
    ...
    {
		//  
		public ISeries<Bar> _barSeries;
		private int _lastIndex = -1;
		public FisherTransformOscillator _ftoIndH1;
		public double sF=0,sF1=0,sdF=0;
		...
		int frDownI = 0, frUpI=0,iSell=0, iBuy=0;
	...
	_lastIndex = _barSeries.Range.To - 1;
	 ...
	 sF = _ftoIndH1.FisherSeries[_lastIndex];
	 sF1 = _ftoIndH1.FisherSeries[_lastIndex-1];
	 sdF=sF-sF1;
		//Print("H1 -- _lastIndex={0}  sF(Delta)={1}  Time={2} ==================================================",_lastIndex,sF,timePrice);	
        }
        ...
        protected override void NewBar()
        {
		  ...
		  if (_frInd.TopSeries[Bars.Range.To-5]>0)    { frUpH=Bars[Bars.Range.To-5].High; frUpL=Bars[Bars.Range.To-5].Low; frUp_Time = Bars[Bars.Range.To-5].Time; frUpI=Bars.Range.To-5;}
		  if (_frInd.BottomSeries[Bars.Range.To-5]>0) { frDownL=Bars[Bars.Range.To-5].Low; frDownH=Bars[Bars.Range.To-5].High; frDown_Time = Bars[Bars.Range.To-5].Time; frDownI=Bars.Range.To-5;}
...
// Сигнал №2  Buy - Цена выше 20-й и 50-й EMA.			
			  ...
			  //if(H20x50 && Bars.Range.To==(NBarH+i) && Bars[Bars.Range.To].Low>SMa20 && i1==0 && i==0 ) 
			  //if(H20x50 && Bars.Range.To==(NBarH+i) && Bars[Bars.Range.To].Low>SMa20 && i1==0 )  
			  if(H20x50 && Bars.Range.To==(NBarH+i) && Bars[Bars.Range.To].Low<SMa20  ) i1++;  
			  if(H20x50) i++;
			  ...
			  //if(L20x50 && Bars.Range.To==(NBarL+j) && Bars[Bars.Range.To].High<SMa20 && j1==0 && j==0 )  {  Print("SELL - sdF={0}<0 =======================",sdF);} 
			  //if(L20x50 && Bars.Range.To==(NBarL+j) && Bars[Bars.Range.To].High<SMa20 && j1==0 ) 
...
// Сигнал №4  Buy - Цена касается 20-й MA.
			...
			if(i>7 &&  i1==1 && Bars[Bars.Range.To-1].Close<SMa20 )  
				 ...
				 if(sdF>0 && i2==3 && Bars.Range.To==(XBarH+3) && Bars[XBarH+3].Close>SMa20  && _positionGuid==Guid.Empty) 
				 { Buy(); pBuy=true; iBuy=Bars.Range.To; i1=0; i3=0; i2=0;    Print("Buy!! - {0} = {1} ",(SMa50-SMa20),Bars[Bars.Range.To-1].Time);} 
			...
			if(j>7 && j1==1 &&  (Bars[Bars.Range.To-1].Close)>SMa20 ) 
				 ...
				 if(sdF<0 && j2==3 && Bars.Range.To==(XBarL+3) && Bars[XBarL+3].Close<SMa20 && _positionGuid==Guid.Empty) 
				 { Sell(); pSell=true; iSell=Bars.Range.To;  j1=0; j3=0; j2=0;  j=0; Print("SEL!! - {0} = {1}",(SMa50-SMa20),Bars[Bars.Range.To-1].Time);}
...
// ==================================================================================================================================
        ...
        //  після перетину свічки фракталу
		if (pBuy && Bars[Bars.Range.To-1].Close<frDownL  && _positionGuid!=Guid.Empty) { ClosePosition(); pBuy=false; i3=0; } 
		if (pSell && Bars[Bars.Range.To-1].Close>frUpH  && _positionGuid!=Guid.Empty) { ClosePosition(); pSell=false; j3=0;}		
		
		//  Після торкання МА 20
		...
		//  Після торкання МА 50
		//	if (pBuy && Bars[Bars.Range.To-1].Close<SMa50 && _positionGuid!=Guid.Empty)  { ClosePosition(); pBuy=false; i3=0; }
		//	if (pSell && Bars[Bars.Range.To-1].Close>SMa50 && _positionGuid!=Guid.Empty) { ClosePosition(); pSell=false; j3=0;}
		
		//
		if (pBuy && frUpI>iBuy  && _positionGuid!=Guid.Empty) { ClosePosition(); pBuy=false; i3=0; } 
		if (pSell && frDownI>iSell  && _positionGuid!=Guid.Empty) { ClosePosition(); pSell=false; j3=0;}		
