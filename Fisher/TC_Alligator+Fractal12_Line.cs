...
{
    [TradeSystem("TC_Alligator+Fractal12_Line")]        //copy of "TC_Alligator+Fractal11"
...
// ==================================================================================================================================		  
			...
			// низ Бар-Фрактала выше Alligator  - Назначаем рабочим (fr_all_Up) для Buy
			  ...
			  if (frSU>0 && frUpL>aGuba5 && frUpL>aChelust5 && frUpL>aZub5)   
			     { fr_all_Up=frUpH; fr_all_Up_Time=frUp_Time;  fr_all_Up_L=true;  ifrU++; ifrD=0;  
					 vlR.Time=Bars[Bars.Range.To-5].Time; HLine();
			...
			// Если появился новый фрактал  ВВЕРХ и касается Алигатора - отменяем "рабочий"
			  if ( frSU>0 && !(frUpL>aGuba5 && frUpL>aChelust5 && frUpL>aZub5) ) { fr_all_Up_L=false; ifrU=0; vlY.Time=Bars[Bars.Range.To-5].Time;
...
//===============================================================================================================================			
			...
			if (frSD>0 && frDownH<aGuba5 && frDownH<aChelust5 && frDownH<aZub5) 
				{ fr_all_Down=frDownL;  fr_all_Down_Time=frDown_Time; fr_all_Down_L=true; ifrD++;ifrU=0;  
					vlB.Time=Bars[Bars.Range.To-5].Time; LLine();
			...
			if (frSD>0 && !(frDownH<aGuba5 && frDownH<aChelust5 && frDownH<aZub5)) { fr_all_Down_L=false; ifrD=0; vlY.Time=Bars[Bars.Range.To-5].Time;
	...
	//if (ifrD>2) Blue();
       ...
       private void Red() 
            { vlR.Time=Bars[Bars.Range.To-1].Time; }
			      private void Blue() 
            {vlB.Time=Bars[Bars.Range.To-1].Time;}
					private void Green() 
			{ vlG.Time=Bars[Bars.Range.To-1].Time;}	
				     private void Yellow() 
            {vlY.Time=Bars[Bars.Range.To-1].Time;}	
    }
}