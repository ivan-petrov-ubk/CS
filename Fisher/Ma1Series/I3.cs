...
namespace IPro.TradeSystems
{
    ...
    public class ZZ_Ex1 : TradeSystem
    {
		...
		private bool isIU=true,isID=false,isIMU=true,isIMD=false;
...
//======================================================================================================================================		  
		    ...
		    if(tank1>kij1 && kij2>=tank2) isIU=true; else isIU=false;
			if(tank1<kij1 && kij2<=tank2) isID=true; else isID=false;	
			// Цена ВЫШЕ/НИЖЕ Senkou Span B
			if(Bars[Bars.Range.To-1].Close>senB1) isIMU=true; else isIMU=false;				
			if(Bars[Bars.Range.To-1].Close<senB1) isIMD=true; else isIMD=false; 
			...
			if(isFU)  {if (posGuidBuy!=Guid.Empty && Trade.GetPosition(posGuidBuy).State==PositionState.Active) 
			{var res1 = Trade.CloseMarketPosition(posGuidBuy); if (res1.IsSuccessful) posGuidBuy = Guid.Empty;}			}
		    if(isFD)  {if (posGuidSell!=Guid.Empty && Trade.GetPosition(posGuidSell).State==PositionState.Active) 
			{var res3 = Trade.CloseMarketPosition(posGuidSell); if (res3.IsSuccessful) posGuidSell = Guid.Empty;}	}           
...
//======================================================================================================================================
	...
	if ( isIU && isIMU && F1U && _ftoInd.FisherSeries[Bars.Range.To-1]>0) {
		   var vline = Tools.Create<VerticalLine>();
           vline.Color=Color.Red;
		   vline.Time=Bars[Bars.Range.To-1].Time;
				vline.Width=4;
						 if(posGuidSell==Guid.Empty){
								var result2 = Trade.OpenMarketPosition(Instrument.Id, ExecutionRule.Buy, 0.1,Instrument.Ask, -1,Stops.InPips(200,null), null, null);
								  if (result2.IsSuccessful)  posGuidSell=result2.Position.Id;  } 
			}
		   
			
	if ( isID && isIMD && F1D && _ftoInd.FisherSeries[Bars.Range.To-1]<0) {
		   var vline = Tools.Create<VerticalLine>();
