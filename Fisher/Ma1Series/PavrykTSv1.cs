...
{
    [TradeSystem("PavrykTSv1")]
    public class PavrykTSv1 : TradeSystem //for M15 Timeframe
    {
...
/// </summary>
		...
		[Parameter("Use_Levels_S1|R1", DefaultValue = true)]
        public bool LevelSR1;
		
		[Parameter("Orders_LOTS", DefaultValue = 0.5)]
        public double vol;
		
		[Parameter("Write full log", DefaultValue = false)]
		...
		private Pivot Pivind;

		private MovingAverage MA5ind;
		private MovingAverage MA24ind;
		
		
		private double FTOFsS;
		private double FTODown;
		private	double FTOUp;
		
		private	double FTOMA1;
		private	double FTOMA2;
		...
		private orderSL currOrderSL=new orderSL(0.0,0.0);
		private orderSL prevOrderSL=new orderSL(0.0,0.0);
		
		private int _CIndex=-1; 
		private int _pivotIndex=-1; 

		private int  magicNumber = 777;

		private bool MA5UpDown = false;		
		private bool MA5DownUp = false;		
        ...
        protected override void NewBar()
        {
				...
				PrintEx(string.Format(" Index = {5}:: FTO >> {0}::{1}:{2}::{3}:{4}",
				...
				PrintEx(string.Format(" Index = {5}:: PVT >> {0}::{1}:{2}::{3}:{4}",
				...
				if(MA5UpDown)
				{
					PrintEx(string.Format("MA5 UP : {0}", MA5DownUp));
					
					if((FTOUp.Equals(double.NaN)) && FTOMA1<FTOMA2)
					{
						PrintEx(string.Format("[S] Signal \"Sell\""+DateTime.Now.ToString("o")));
					setMarketSellOrder();
					}
				}
				...
				if(MA5DownUp)
				{
					PrintEx(string.Format("MA5 Dn : {0}", MA5UpDown));
					if((FTODown.Equals(double.NaN)) && FTOMA1>FTOMA2)
					{
						PrintEx(string.Format("[B] Signal \"Buy\""+DateTime.Now.ToString("o")));
					setMarketBuyOrder();
					}
				}
				PrintEx(string.Format("[-]bSL:{0},[+]sSL:{1}",currOrderSL._BuySL,currOrderSL._SellSL));
		...
		protected void TrailActivePositions()
		{
			...
			if(currOrderSL._BuySL!=prevOrderSL._BuySL || currOrderSL._SellSL!=prevOrderSL._SellSL)
			{
				...
				for(int npos=lps.Length-1;npos>=0;npos--)
				{
					if((int)lps[npos].Type==(int)ExecutionRule.Buy)
					{
						...
						do
							{ 
							attempts++;
							tR=Trade.UpdateMarketPosition(lps[npos].Id,currOrderSL._BuySL,(double)Math.Round((double)lps[npos].TakeProfit,Instrument.PriceScale));
							} 
						while ((tR==null?true:!tR.IsSuccessful) && attempts<100);
						}
					}
					if((int)lps[npos].Type==(int)ExecutionRule.Sell)
					{
						...
						do
							{ 
							attempts++;
							tR=Trade.UpdateMarketPosition(lps[npos].Id,currOrderSL._SellSL,(double)Math.Round((double)lps[npos].TakeProfit,Instrument.PriceScale));
							} 
						while ((tR==null?true:!tR.IsSuccessful) && attempts<100);
						}
					}
					...
					{
						PrintEx(string.Format("[*E*]{2} Error in modification of {1}-position {0}",
				...
				if(count>0)
				{
						PrintEx(string.Format("[!]{1} Successfully modified {0} positions",
								count,DateTime.Now));
				}
			}
		...
		protected void setMarketSellOrder()
		{
			Stops SLTP=Stops.InPrice(
									currOrderSL._SellSL,
									dblCalculateTP(ExecutionRule.Sell,0)
									);
			int attempt=0;
			TradeResult tR=null;
			do 
			...
			if((tR==null?true:!tR.IsSuccessful))
			{
				PrintEx(string.Format("[{2}>>]{1} Cann't Send  Sell order  at Bid: {0} ",
					Instrument.Bid,DateTime.Now,Instrument.Name));
			}
			else
			{
				PrintEx(string.Format("[{3}>>]{2} Sended successfully Sell order for position {0} at Bid: {1} ",
		...
		protected void setMarketBuyOrder()
		{
			Stops TPSL=Stops.InPrice(
									currOrderSL._BuySL,
									dblCalculateTP(ExecutionRule.Buy,0)
									);
			int attempt=0;
			TradeResult tR=null;
			do 
			...
			if((tR==null?true:!tR.IsSuccessful))
			{
				PrintEx(string.Format("[{2}>>]{1} Cann't Send Buy order at Ask: {0} ",
					Instrument.Bid,DateTime.Now,Instrument.Name));
			}
			else
			{
				PrintEx(string.Format("[{3}>>]{2} Sended successfully Buy order for position {0} at Ask: {1} ",
					tR.Position.Id,tR.Position.OpenPrice,tR.Position.OpenTime,Instrument.Name));
			}
			
		}
		
		private void PrintEx(string logMessage)
		{
