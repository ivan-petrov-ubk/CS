...
namespace IPro.Samples
{
    ...
    public sealed class SampleFisherTransformOscillator : Indicator
    {
        ...
        protected override void Calculate(int index)
        {
			...
			if( lastIndex!=index) {
			if (fOld>=0 && fish<0) System.Console.Beep(800,500);
			if (fOld<0 && fish>=0) System.Console.Beep(1200,500);				
				
			if (fOld>=0 && fish<0) { Print("Пара-{0} - fOld={1} fish={2} - {3} - {4}-{5} {6}",Instrument.Name, fOld, fish, Bars[index].Time,Dt,DTime,index); vlR.Time=Bars[index].Time; //System.Console.Beep(800,500);
			}  
			if (fOld<0 && fish>=0) { Print("Пара-{0} - fOld={1} fish={2} - {3} - {4}-{5} {6}",Instrument.Name, fOld, fish, Bars[index].Time,Dt,DTime,index); vlB.Time=Bars[index].Time; // System.Console.Beep(1200,500);
			}
