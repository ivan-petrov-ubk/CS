...
namespace IPro.TradeSystems
{
	...
	public class Elder : TradeSystem
    {
		...
		private double maxu=0,mind=1000,maxd=0,minu=1000,rz1,rz2,cn;
...
//=== КОРЕКЦИЯ ===========================================================================================================							 
			...
			{   kl=0; mind=1000; maxd=0;
				do
				{ kl++; if(Bars[Bars.Range.To-kl].Low<mind) mind=Bars[Bars.Range.To-kl].Low;
				        if(Bars[Bars.Range.To-kl].High>maxd) maxd=Bars[Bars.Range.To-kl].High;}
				...
				Print("{0}  {1} > {2} = {3} ",Bars[Bars.Range.To-1].Time,Bars[Bars.Range.To-1].High,maxd,Bars[Bars.Range.To-1].High>maxd);
				if(tu && Bars[Bars.Range.To-1].High>maxd) Buy1();		
			} 
            // DOWN - ВНИЗ - ВИЗНАЧАЄМО МАКСИМУМ
			if(_ftoInd.FisherSeries[Bars.Range.To-1]<-0.3 &&  _ftoInd.FisherSeries[Bars.Range.To-2]>-0.3  ) 
			{   kl=0;maxu=0; minu=1000;
				do
				{ kl++; if(Bars[Bars.Range.To-kl].High>maxu) maxu=Bars[Bars.Range.To-kl].High;  
						if(Bars[Bars.Range.To-kl].Low<minu) minu=Bars[Bars.Range.To-kl].Low; }
				while(!(_ftoInd.FisherSeries[Bars.Range.To-kl]>-0  &&  _ftoInd.FisherSeries[Bars.Range.To-kl-1]<-0));
				max4=max3;max3=max2;max2=max1; max1=maxu;
				Print("{0}  {1} > {2} = {3} ",Bars[Bars.Range.To-1].Time,Bars[Bars.Range.To-1].Low,minu,Bars[Bars.Range.To-1].Low<minu);
				if(td && Bars[Bars.Range.To-1].Low<minu) Sell1();
		     }
					TrailActiveOrders1();
