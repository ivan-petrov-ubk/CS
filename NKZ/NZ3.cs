...
// Определение патерна   ПИК ПОСЛЕДНЕГО ФРАКТАЛА ПАТЕРНА ВНИЗУ 
		   ...
		   { 
           		if( frU3>frU2 && frU1>frU2 ) {
				    ...
				    XXPrint("BUY -- {0} -- TP={1} SL={2} TP/SL={3} TP(п)={4} SL(p)={5} ",Bars[Bars.Range.To-1].Time,PRU,UBU,PCU,PRUi,UBUi);
			...
			{ 
				...
				{ 
					...
					XXPrint("SELL -- {0} -- TP={1} SL={2} TP/SL={3} TP(п)={4} SL(p)={5} ",Bars[Bars.Range.To-1].Time,PRD,UBD,PCD,PRDi,UBDi);
