...
namespace IPro.TradeSystems
{
    ...
    public class ZZ_Ex1 : TradeSystem
    {
        ...
        {	
			...
			if (Instrument.Name == "USDJPY")  NKZ=544;
			if (Instrument.Name == "USDCAD")  NKZ=528;
			if (Instrument.Name == "USDCHF")  NKZ=534;
...
//====  Fractal ====================================================================================
			  ...
			  if (_frInd.TopSeries[Bars.Range.To-frac]>0) 		{	
				  ...
				  XXPrint("Fractal ВВЕРХ - {0} - frU1={1} frU2={2} frU3={3} frD1={4} frD2={5} frD3={6}",Bars[Bars.Range.To-frac].Time,frU1,frU2,frU3,frD1,frD2,frD3);
				  }
			  if (_frInd.BottomSeries[Bars.Range.To-frac]>0)   { 
				  ...
				  XXPrint("Fractal ВНИЗ - {0} - frU1={1} frU2={2} frU3={3} frD1={4} frD2={5} frD3={6}",Bars[Bars.Range.To-frac].Time,frU1,frU2,frU3,frD1,frD2,frD3);
				  } 			
...
// Определение патерна   ПИК ПОСЛЕДНЕГО ФРАКТАЛА ПАТЕРНА ВНИЗУ 
		   ...
		   { 
           		if( frU3>frU2 && frU1>frU2 && frD3!=0 && frD2!=0) {
					...
					if(UBU>40 && UBU<SL1 && PCU>2 && Bars[ci].Close>nkz2) {
...
// Определение патерна   ПИК ПОСЛЕДНЕГО ФРАКТАЛА ПАТЕРНА ВВЕРХУ 
			...
			{ 
				...
				{ var vl1 = Tools.Create<VerticalLine>(); vl1.Time=Bars[ci].Time; vl1.Color=Color.Aqua;vl1.Width=2;
					...
					if(UBD>40 && UBD<SL1 && PCD>2 && Bars[ci].Close<nkz2) {
