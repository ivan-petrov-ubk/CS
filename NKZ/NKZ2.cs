...
namespace IPro.TradeSystems
{
    ...
    public class ZZ_Ex1 : TradeSystem
    {
        ...
        {	
			...
			if (Instrument.Name == "USDJPY") {  NKZ=547;  mgB=105; mgS=205; }
			if (Instrument.Name == "USDCAD") {  NKZ=530;  mgB=106; mgS=206; }
			if (Instrument.Name == "USDCHF") {  NKZ=536;  mgB=107; mgS=207; }
			...
			if (Instrument.Name == "EURCAD") {  NKZ=726;  mgB=113; mgS=213; } 
			if (Instrument.Name == "EURCHF") {  NKZ=682;  mgB=114; mgS=214; }
...
Print("U ИТОРИЯ- {0} - frU1={1} frU2={2} frU3={3} frD1={4} frD2={5} frD3={6} i={7}", Bars[Bars.Range.To-frac-i].Time,frU1,frU2,frU3,frD1,frD2,frD3,i);
				  }
...
Print("D ИСТОРИЯ - {0} - frU1={1} frU2={2} frU3={3} frD1={4} frD2={5} frD3={6} i={7}", Bars[Bars.Range.To-frac-i].Time,frU1,frU2,frU3,frD1,frD2,frD3,i);				  
			  	} 			
            i++;
        }		
...
//====  Fractal ====================================================================================
			  if (_frInd.TopSeries[Bars.Range.To-frac]>0) 		{	
				 ...
				 Print("{0} - frU1={1} frU2={2} frU3={3} frD1={4} frD2={5} frD3={6} nkz4={7} nkz2={8} tu={9} nu={10}", Bars[Bars.Range.To-frac].Time,frU1,frU2,frU3,frD1,frD2,frD3,nkz4,nkz2,tu,nu);
				  }
...
Print("{0} - frU1={1} frU2={2} frU3={3} frD1={4} frD2={5} frD3={6} nkz4={7} nkz2={8} td={9} nd={10}", Bars[Bars.Range.To-frac].Time,frU1,frU2,frU3,frD1,frD2,frD3,nkz4,td,nd);
				  } 			
		  ...
		  { 			 // Индекс свечипоследнего
			...
			if(tu) { zmax = dt1*Instrument.Point;  
					 ...
					 nkz2v=Math.Round(nkz2v,Instrument.PriceScale); 		
				 	 Print("Первая прорисовка ВВЕРХ - {0} - nkz2={1} nkz4={2} - zmax={3}",Bars[ci].Time,nkz2,nkz4,zmax);
                   }
			...
			if(td) { zmin =  dt1*Instrument.Point;
					 ...
					 nkz4v=Math.Round(nkz4v,Instrument.PriceScale);  
					 nkz2v=Math.Round(nkz2v,Instrument.PriceScale);  
				 	 Print("Первая прорисовка ВНИЗ - {0} - nkz2={1} nkz4={2} - zmin={3}",Bars[ci].Time,nkz2,nkz4,zmin);
  				   }
...
//====================================================================================================================      		
	  if(k>0) {
		  if (tu && Bars[ci].High>zmax) { 
			         ...
			         nkz4v=Math.Round(nkz4v,Instrument.PriceScale);  
			         nkz2v=Math.Round(nkz2v,Instrument.PriceScale); 
 			Print("ПЕРЕРИСОВКА ВВЕРХ - {0} - nkz2={1} nkz4={2} - 1U",Bars[ci].Time,nkz2,nkz4);
			toolRectangle.Point1=new ChartPoint(Bars[ci].Time, nkz4);
		  ...
		  if (td && Bars[ci].Low<zmin ) { 
			        ...
			        nkz4v=Math.Round(nkz4v,Instrument.PriceScale);  
			        nkz2v=Math.Round(nkz2v,Instrument.PriceScale); 
 			Print("ПЕРЕРИСОВКА ВНИЗ - {0} - nkz2={1} nkz4={2} - 1U",Bars[ci].Time,nkz2,nkz4);
			toolRectangle.Point1=new ChartPoint(Bars[ci].Time, nkz4);
          	toolRectangle.Point2=new ChartPoint(Bars[ci].Time.AddHours(48), nkz4v);
			 nd=false; nu=false; 			  
		    toolRectangle1.Point1=new ChartPoint(Bars[ci].Time, nkz2);
...
//== Касание зоны/1\4 ======================================================================================================
		  if(n4)  {
			if (tu && Bars[ci].Low<nkz4 && !nu) { nu=true; Print("ПЕРЕСЕЧЕНИЕ 1/4 ВНИЗ - {0} - {1}",Bars[ci].Time,nkz4);}
			if (td && Bars[ci].High>nkz4 && !nd) { nd=true; Print("ПЕРЕСЕЧЕНИЕ 1/4 ВВЕРХ - {0} - {1}",Bars[ci].Time,nkz4);}
                  } 
			if (tu && Bars[ci].Low<nkz2 && !nu) { nu=true; Print("ПЕРЕСЕЧЕНИЕ 1/2 ВНИЗ - {0} - {1}",Bars[ci].Time,nkz2);}
			if (td && Bars[ci].High>nkz2 && !nd) { nd=true; Print("ПЕРЕСЕЧЕНИЕ 1/2 ВВЕРХ - {0} - {1}",Bars[ci].Time,nkz2);}
...
// Определение патерна   ПИК ПОСЛЕДНЕГО ФРАКТАЛА ПАТЕРНА ВНИЗУ 
		   ...
		   { 
           		if( frU3>frU2 && frU1>frU2 && frU1>0 && frU2>0 && frU3>0 ) {
					...
					Print("PATERN BUY -- {0} -- TP={1} SL={2} TP/SL={3} Fract 1-{4} 2-{5} 3-{6} Ot={7}",Bars[ci].Time,PRUi,UBUi,PCU,tmU1,tmU2,tmU3,Ot);
					if(UBU>20 && UBU<SL1 && PCU>2 && Bars[ci].Close>nkz2) {
				    Print("BUY -- {0} -- TP={1} SL={2} TP/SL={3} Fract 1-{4} 2-{5} 3-{6} Ot={7}",Bars[ci].Time,PRU,UBU,PCU,tmU1,tmU2,tmU3,Ot);
...
// Определение патерна   ПИК ПОСЛЕДНЕГО ФРАКТАЛА ПАТЕРНА ВВЕРХУ 
			...
			{ 
				...
				{   var vl1 = Tools.Create<VerticalLine>(); vl1.Time=Bars[ci].Time; vl1.Color=Color.Aqua;vl1.Width=2;
					...
					Print("PATERN SELL -- {0} -- TP(п)={1} SL(p)={2} TP/SL={3} Fract 1-{4} 2-{5} 3-{6}  Ot={7}",Bars[ci].Time,PRDi,UBDi,PCD,tmD1,tmD2,tmD3,Ot);				
					if(UBD>20 && UBD<SL1 && PCD>2 && Bars[ci].Close<nkz2) {
					Print("SELL -- {0} -- TP={1} SL={2} TP/SL={3} Fract 1-{4} 2-{5} 3-{6} Ot={7}",Bars[ci].Time,PRDi,UBDi,PCD,tmD1,tmD2,tmD3,Ot);
