...
namespace IPro.TradeSystems
{
    ...
    public class ZZ_Ex1 : TradeSystem
    {
        ...
        {	
			...
			if (Instrument.Name == "GBPUSD") { NKZ=704;  mgB=102; mgS=202; }
			if (Instrument.Name == "AUDUSD") { NKZ=343;  mgB=103; mgS=203; }
			if (Instrument.Name == "NZDUSD") { NKZ=302;  mgB=104; mgS=204; }
			if (Instrument.Name == "USDJPY") { NKZ=536;  mgB=105; mgS=205; }
			if (Instrument.Name == "USDCAD") { NKZ=542;  mgB=106; mgS=206; }
			if (Instrument.Name == "USDCHF") { NKZ=597;  mgB=107; mgS=207; }
...
//===========================================================================      		
 ...
 Print("{0} - nkz2={1} nkz4={2} - 2D",Bars[Bars.Range.To-1].Time,nkz2,nkz4);
			...
			//== Касание зоны/1\4 ===============================
		  if(n4) {
			if (tu && Bars[ci].Close<nkz4 && !nu)  { nu=true; Print("{0} -Перетин 1/4 ВНИЗ",Bars[Bars.Range.To-1].Time); }
			if (td && Bars[ci].Close>nkz4 && !nd) { nd=true; Print("{0} -Перетин 1/4 ВВЕРХ",Bars[Bars.Range.To-1].Time); }
                  } 
			if (tu && Bars[ci].Close<nkz2 && !nu)  { nu=true;  Print("{0} -Перетин 1/2 ВНИЗ",Bars[Bars.Range.To-1].Time);}
			if (td && Bars[ci].Close>nkz2 && !nd) { nd=true;  Print("{0} -Перетин 1/2 ВВЕРХ",Bars[Bars.Range.To-1].Time);}
