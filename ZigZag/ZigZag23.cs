...
namespace IPro.TradeSystems
{
    ...
    public class ZigZag2 : TradeSystem
    {
        ...
        protected override void NewBar()
        {
			...
			if(Bars[zzd3].Low>Bars[zzd1].Low &&  Bars[zzd3].High<Bars[zzd2].High && Bars[zzd1].High<Bars[zzd2].High) 
			{
			...
			if(posGuidSell==Guid.Empty) {
			   var result1 = Trade.OpenPendingPosition(Instrument.Id, ExecutionRule.SellLimit, 0.1,  Bars[zzd2].High, 0, Stops.InPips(200,100), null, null, null);
			...
			if(Bars[zzd3].High<Bars[zzd1].High &&  Bars[zzd3].Low>Bars[zzd2].Low && Bars[zzd1].Low>Bars[zzd2].Low) 
			{
				var toolVerticalLine=Tools.Create<VerticalLine>();
     			toolVerticalLine.Time = Bars[zzd2].Time;
				toolVerticalLine.Color=Color.Blue;
			    if(posGuidBuy==Guid.Empty) {
				var result = Trade.OpenPendingPosition(Instrument.Id, ExecutionRule.BuyLimit, 0.1, Bars[zzd2].Low, 0, Stops.InPips(200,100), null, null, null);
			...
			if (posGuidBuy!=Guid.Empty && Trade.GetPosition(posGuidBuy).State==PositionState.Pending)
				{
					...
					var result1 = Trade.OpenPendingPosition(Instrument.Id, ExecutionRule.BuyLimit, 0.1,  Bars[zzd2].Low, 0, Stops.InPips(200,100), null, null, null);
