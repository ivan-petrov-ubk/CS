...
namespace IPro.TradeSystems
{
    ...
    public class Wiliams_Svechi : TradeSystem
    {
        ...
        protected override void NewBar()
        {
			...
			// Определяем максимальный размер тела 100 свечок
			MaxB=0;Sr=0;
			for (int i=1; i<100; i++) {  
				  Body=Bars[Bars.Range.To-i].High-Bars[Bars.Range.To-i].Low; 
				  if (Body>MaxB) MaxB=Body; 
			   Sr=Sr+Body;
			}
			Sr=Sr/100;
			MaxB=Math.Round(MaxB,5);
			Sr=Math.Round(Sr,5);
			//Print("Max={0} Среднее={1}  -- {2}",MaxB,Sr,Bars[Bars.Range.To-1].Time);
			// 2 Схема 
			Body= Bars[Bars.Range.To-1].High-Bars[Bars.Range.To-1].Low;
			// Доджи
			//if(BarC1<BarO1 && BarC1<BarL1+(Body/3) &&  BarO1<BarL1+(Body/3)) { vlR.Time=DTime1;Sell();}
			//if(BarC1>BarO1 && BarC1<BarL1+(Body/3) &&  BarO1<BarL1+(Body/3)) { vlB.Time=DTime1;Buy();}
			//if(BarC1<BarO1 && BarC1>BarH1-(Body/3) &&  BarO1>BarH1-(Body/3)) { vlR.Time=DTime1;Buy();}
			//if(BarC1>BarO1 && BarC1>BarH1-(Body/3) &&  BarO1>BarH1-(Body/3)) { vlB.Time=DTime1;Sell();}
			
			// Большое тело
			if(BarC1>BarO1 && (BarC1-BarO1)>Sr && BarO1<(BarL1+(Body/3)) && BarC1>(BarH1-(Body/3))) { vlR.Time=DTime1;Buy();}
			if(BarC1<BarO1 && (BarO1-BarC1)>Sr && BarC1<(BarL1+(Body/3)) && BarO1>(BarH1-(Body/3))) { vlB.Time=DTime1;Sell();}
			//if(BarC1>BarO1 && BarC1<BarL1+(Body/3) &&  BarO1<BarL1+(Body/3)) { vlB.Time=DTime1;Buy();}
			//if(BarC1<BarO1 && BarC1>BarH1-(Body/3) &&  BarO1>BarH1-(Body/3)) { vlR.Time=DTime1;Buy();}
			//if(BarC1>BarO1 && BarC1>BarH1-(Body/3) &&  BarO1>BarH1-(Body/3)) { vlB.Time=DTime1;Sell();}
