...
{
    [TradeSystem("Bola_No3B")]
    public class Bola_No2 : TradeSystem
    {
...
/// </summary>
		...
		[Parameter("Log_Path", DefaultValue = @"BoLaLoG")]
...
#region	GetIndicatorsAndFrames // Get Indicators' references...           
		...
		protected void TrailPendingOrders()
		{
			...
			foreach(Guid pos in listOfBuyStop)
			{
				...
				if(Mode3b)
				{
					OP = CalculateOpenPrice3B(ExecutionRule.BuyStop);
				}
				else
				{
			...
			foreach(Guid pos in listOfSellStop)
			{
				...
				if(Mode3b)
				{
					OP = CalculateOpenPrice3B(ExecutionRule.SellStop);
				}
				else
				{
