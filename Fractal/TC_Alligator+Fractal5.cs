...
namespace IPro.TradeSystems
{
	...
	_lastIndex = _barSeries.Range.To - 1;
	 ...
	 rF = _ftoIndH1.FisherSeries[_lastIndex];
	 rF1 = _ftoIndH1.FisherSeries[_lastIndex-1];
	 rdF=rF-rF1;	
        }
...
// Сигнал №2  Цена за алигатором
			...
			//if (aH && Bars.Range.To==(NBarH+i) && Bars[Bars.Range.To-1].Low>aGuba && i1==0)  Red();
            // Для Sell
			...
			//if (aL && Bars.Range.To==(NBarL+j) && Bars[Bars.Range.To-1].High<aGuba && j1==0)  Blue();
...
// Сигнал №3  Линии алигатора разошлись - Растояние между линиями больше 0.001
			...
			if (aH && Bars.Range.To==(NBarH+i) && ((aGuba-aChelust)>0.001) ) { aUgolH=true; Red();}
            // Для Sell
			//if (aL) Print("SELL - {0}>5 - {1}",(aGuba-aChelust)*10000,Bars[Bars.Range.To-1].Time);
			if (aL && Bars.Range.To==(NBarL+j) && ((aChelust-aGuba)>0.001)) { aUgolL=true; Blue();}
...
// Сигнал №4  - Для Buy - Цена ЗАКРЫЛАСЬ внутри алигатора  
			//if (aH && aUgolH) Print("BUY -- Bars.Range.To{0}==(NBarH+i){1} && Bars[Bars.Range.To].Close{2}<aGuba{3} - {4}",Bars.Range.To,(NBarH+i),Bars[Bars.Range.To].Close,aGuba,Bars[Bars.Range.To].Time);
     		if (i>1 && aH && aUgolH && Bars.Range.To==(NBarH+i) && Bars[Bars.Range.To].Close<aGuba ) { i1++; }
			...
			//if (aL && aUgolL) Print("Sell -- Bars.Range.To{0}==(NBarH+j){1} && Bars[Bars.Range.To].Close{2}>aGuba{3} - {4}",Bars.Range.To,(NBarL+j),Bars[Bars.Range.To].Close,aGuba,Bars[Bars.Range.To].Time);
 			if (j>1 && aL && aUgolL && Bars.Range.To==(NBarL+j) && Bars[Bars.Range.To].Close>aGuba ) { j1++; }
...
// Сигнал №5  - Для AO - Сигнал БЛЮДЦЕ
			//  Блюдце - низинка
		     // Для Buy
			  if (rdF>0 && aH && Bars.Range.To==(NBarH+i) && i1>0 && aoDown2>0 && aoUp>0 && aoUp1>0 && _positionGuid==Guid.Empty)  { Buy(); Red(); aH=false; aUgolH=false; pBuy=true; }    // Зеленый - красный
		     // Sell 
			  if (rdF<0 && aL && aUgolL && Bars.Range.To==(NBarL+j) && aoUp2<0 && aoDown<0 && aoDown1<0 && _positionGuid==Guid.Empty)  { Sell(); Blue(); aL=false; aUgolL=false; pSell=true; } // Красный - зеленый 
