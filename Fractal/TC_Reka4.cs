...
// Сигнал №4  Buy - Цена касается 20-й MA.
			...
			if(i>7 &&  i1==1 && Bars[Bars.Range.To-1].Close<SMa20 )  
				 ...
				 {Blue();Buy(); i1=0; i3=0; i2=0; pBuy=true;   Print("Buy!! - {0} = {1} ",(SMa50-SMa20),Bars[Bars.Range.To-1].Time);} 
			...
			if(j>7 && j1==1 &&  (Bars[Bars.Range.To-1].Close)>SMa20 ) 
				 ...
				 {Red(); Sell(); j1=0; j3=0; j2=0; pSell=true;  j=0; Print("SEL!! - {0} = {1}",(SMa50-SMa20),Bars[Bars.Range.To-1].Time);}
		...
		//	
			if (pBuy && i3>5 && Bars[Bars.Range.To-1].Close<SMa20 && _positionGuid!=Guid.Empty)  { ClosePosition(); pBuy=false; i3=0; }
		//	     Print("{0} -- Bars[Bars.Range.To-1].Close{1}<SMa50{2}",Bars[Bars.Range.To-1].Time,Bars[Bars.Range.To-1].Close,SMa50); }
			if (pSell && j3>5 && Bars[Bars.Range.To-1].Close>SMa20 && _positionGuid!=Guid.Empty) { ClosePosition(); pSell=false; j3=0;}
