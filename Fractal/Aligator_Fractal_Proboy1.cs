...
//				  if (fSell && _positionGuid==Guid.Empty) { Blue(); }
				 ...
				 if(pBay && Bars[Bars.Range.To-1].High>aGuba && _positionGuid!=Guid.Empty) { ClosePosition();  pBay=false;}
				 // Закрываем ордер когда прибыль > 150п
				 //if(pBay && (Bars[Bars.Range.To-1].Close-OpenPrice)>0.0015 && _positionGuid!=Guid.Empty) { ClosePosition(); pBay=false; }
				 ...
				 if(pSell && Bars[Bars.Range.To-1].Low<aGuba && _positionGuid!=Guid.Empty) { ClosePosition();  pSell=false; }
				 // Закрываем ордер когда прибыль > 150п
			     //if(pSell && (OpenPrice-Bars[Bars.Range.To-1].Close)>0.0015 && _positionGuid!=Guid.Empty) { ClosePosition(); pSell=false; }
				 //Print("OK! ===================================================================================")
