...
{
    [TradeSystem("ZZ12")]    //copy of "ZZ11"
    public class ZigZag2 : TradeSystem
    {
        // Simple parameter example
        [Parameter("Some comment", DefaultValue = "TC ZZ12")]
        public string CommentText { get; set; }
		[Parameter("TakeProfit:", DefaultValue = 600)]
		...
		[Parameter("Дельта:", DefaultValue = 10)]
		...
		[Parameter("Log_FileName:", DefaultValue = @"ZZ12_")]
        ...
        protected override void NewBar()
        {
		    ...
		    if(posActiveMine.Length>0) {				
				  for (int i=0; i<posActiveMine.Length; i++) 
				  {
					 // if(posActiveMine[i].Type.IsBuy()  && zz3<zz2 && zz2>zz1) { var res = Trade.CloseMarketPosition(posActiveMine[i].Id);}
					 // if(posActiveMine[i].Type.IsSell() && zz3>zz2 && zz2<zz1) { var res = Trade.CloseMarketPosition(posActiveMine[i].Id);}

if(posActiveMine[i].Type.IsBuy()  && Bars[zzd2].High<Bars[zzd1].High && Bars[zzi1].High<Bars[zzd1].High && zzd2>zzTB) { var res = Trade.CloseMarketPosition(posActiveMine[i].Id);}
if(posActiveMine[i].Type.IsSell() && Bars[zzd1].Low<Bars[zzd2].Low && Bars[zzd1].Low<Bars[zzi1].Low  && zzd2>zzTS) { var res = Trade.CloseMarketPosition(posActiveMine[i].Id);}
			  ...
			  { //Print("Вверху Максимум"); 
				...
				if(Bars[zzd1].Low!=zzU) { zzU=Bars[zzd1].Low;	zzTS= Bars.Range.To;
			   	...
			   	var result1 = Trade.OpenPendingPosition(Instrument.Id, ExecutionRule.SellLimit, 0.1,  Bars[zzd1].High-Instrument.Spread, 0, Stops.InPips(SL,TP), Bars[zzd1].Time.AddHours(15), null, null);
						XXPrint("SellLimit {0} {1} {2} {3} {4} {5} {6} {7} {8}",
						Bars[zzd1].Time,
			...
			if(Bars[zzd2].High<Bars[zzi1].High &&  Bars[zzd1].Low<Bars[zzd2].Low && Bars[zzd1].Low<Bars[zzi1].Low &&  Bars[zzi1].Low-Bars[zzd3].Low>kor2) 
				{  //Print("Внизу Максимум");
					vr.Time = Bars[zzd2].Time;
			  		vb.Time = Bars[zzd1].Time;
					vy.Time = Bars[zzi1].Time;
					if(Bars[zzd1].Low!=zzD) { zzD=Bars[zzd1].Low; zzTB= Bars.Range.To;
					...
					XXPrint("BuyLimit {0} {1} {2} {3} {4} {5} {6} {7} {8}",
						Bars[zzd1].Time,
