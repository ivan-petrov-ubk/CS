...
// 1   
     ...
     {  
		 ...
		 if(B1-R2>3 && R1-B1>3 && U1<U2  && Bars[R2].High<Bars[R1].High) {
				...
				toolVerticalLine.Color=Color.Red;
		          ...
		          var result = Trade.OpenMarketPosition(Instrument.Id, ExecutionRule.Sell,  1.0, Instrument.Ask, -1, Stops.InPips(100,150), null, null);	
...
// 3
	 if(aoDown4<-0.0002 && aoDown3<-0.0002 && aoUp2<-0.0002 && aoUp1<-0.0002) 
     {
		 		...
		 		if(Y1-W2>3 && W1-Y1>3 && D2<D1 && Bars[W2].Low>Bars[W1].Low) {
				Print("W2={0} Y1={1} W1={2}  - {3}",W2,Y1,W1,Bars[Bars.Range.To-1].Time);   	
		        var toolVerticalLine=Tools.Create<VerticalLine>();
     			toolVerticalLine.Time = Bars[Bars.Range.To-1].Time;
				toolVerticalLine.Color=Color.White; 
								if (_positionGuidB==Guid.Empty) {
				var result1 = Trade.OpenMarketPosition(Instrument.Id, ExecutionRule.Buy, 1.0, Instrument.Bid, -1, Stops.InPips(100,150), null, null);	
