...
{
    [TradeSystem("NM1_v2")]                 //copy of "NF2_v1"
    public class ZZ_Ex1 : TradeSystem
    {
		...
		private bool tu,td,nu,nd,n2,torg;
		private int NKZ,i;
		...
		public TimeSeriesForecast _tsfInd;
		public MovingAverage _ma50;	
		...
		//private static string PathToLogFile = Environment.CurrentDirectory;
		//private static string PathToLogFile = @"f:";
...
//========  начальная инициализация	==================================================================		
			if(k==0) { InitFr();var vline1 = Tools.Create<VerticalLine>();vline1.Color=Color.Aqua;vline1.Time=DTime;}
			if(k==1) 
		    { 
				if(tu) {
					vl.Time = tmax;vl.Color=Color.Red;
					 ...
					 //XXPrint(" Init - {0} - frU1={1}({4}) frU2={2}({5}) frU3={3}({6}) zmax={7}({8})",	Bars[Bars.Range.To-1].Time,frU1,frU2,frU3,tmU1,tmU2,tmU3,zmax,tmax);
				...
				if(td) { 
					vl.Time = tmin; vl.Color=Color.Blue;
					 ...
					 nkz4v=Math.Round(nkz4v,Instrument.PriceScale);  
					 nkz2v=Math.Round(nkz2v,Instrument.PriceScale);
					 //XXPrint(" Init - {0} - frD1={4}({1}) frD2={5}({2}) frD3={6}({3})  zmin={7}({8})", Bars[Bars.Range.To-1].Time,tmD1,tmD2,tmD3,frD1,frD2,frD3,zmin,tmin);
				    hl.Price=nkz4v;hl.Text=Math.Round((nkz4-zmin)*Math.Pow(10,Instrument.PriceScale),0).ToString();
			}
 		    }
		    k++;	
...
//================================ Определение патерна   ПИК ПОСЛЕДНЕГО ФРАКТАЛА ПАТЕРНА ВНИЗУ =====================
		   if (_tsfInd.MainSeries[Bars.Range.To-1]>=_ma50.SeriesMa[Bars.Range.To-1] && 
			   _tsfInd.MainSeries[Bars.Range.To-3]<=_ma50.SeriesMa[Bars.Range.To-3])
				{    Print("BUY!!!!!!!!!!!!!!!!!!");
					TPU=zmax;
					...
					UBUi=(int)UBU;
					//UBUi=(int)SL1;
					//Print("{2} - {0} - PATERN Top {1} - {3} kof={4} sl={5} tp={6} fsU1={7} fsU2={8} fsU3={9}",
					//Bars[Bars.Range.To-frac].Time,_frInd.TopSeries[Bars.Range.To-frac],Instrument.Name,Bars[Bars.Range.To-1].Time,PCU,UBUi,PRUi,
					//Math.Round(fsU1, 2),Math.Round(fsU2, 2),Math.Round(fsU3, 2));
					
					//if(UBU>20 && UBU<SL1 && Bars[ci].Close>nkz2 && PCU>kof  && fsU1>0 && fsU2<0 && torg)
					Print("torg={0}",torg);
					if(torg)
					{  
		   ...
		   if (_tsfInd.MainSeries[Bars.Range.To-1]<=_ma50.SeriesMa[Bars.Range.To-1] && 
			   _tsfInd.MainSeries[Bars.Range.To-3]>=_ma50.SeriesMa[Bars.Range.To-3])
				{     Print("SELL!!!!!!!!!!!!!!");
					...
					//Print("{2} - {0} PATERN Down {1} - {3}  kof={4} sl={5} tp={6} fsD1={7} fsD2={8} fsD3={9}",
					//Bars[Bars.Range.To-frac].Time,_frInd.BottomSeries[Bars.Range.To-frac],Instrument.Name,Bars[Bars.Range.To-1].Time,PCD,UBDi,PRDi,
					//Math.Round(fsD1, 2),Math.Round(fsD2, 2),Math.Round(fsD3, 2)); 
					//if(UBD>20 && UBD<SL1 && Bars[ci].Close<nkz2 && PCD>kof && fsD1<0 && fsD2>0 && torg) 
					if(torg)
					{	
...
//======  Логи  =====================================================================================================================   
	...
	XPrint("{1} | {0} | frD= {2}:{3}({4}) {5}:{6}({7}) {8}:{9}({10}) | min={11}({13}) nkz4={12}",Bars[Bars.Range.To-1].Time,st2,
				tmD3.Hour,tmD3.Minute,frD3,tmD2.Hour,tmD2.Minute,frD2,tmD1.Hour,tmD1.Minute,frD1,zmin,nkz4,tmin);			
			}
	}
...
//====== History =================================================================================================		
 		protected void InitFr()
		{
			...
			while ( i<kl ) 
			{ 
				if (!_frInd.TopSeries[Bars.Range.To-i].Equals(double.NaN)  && kfu<3)	{  kfu++;
				  frU1=frU2; frU2=frU3; frU3=_frInd.TopSeries[Bars.Range.To-i];
				  fsU1=fsU2; fsU2=fsU3; fsU3=_ftoInd.FisherSeries[Bars.Range.To-i];
				  tmU1=tmU2; tmU2=tmU3; tmU3=Bars[Bars.Range.To-i].Time;  
					  }

			  if (!_frInd.BottomSeries[Bars.Range.To-i].Equals(double.NaN) && kfd<3)   { kfd++; 
				  frD1=frD2; frD2=frD3; frD3=_frInd.BottomSeries[Bars.Range.To-i];
				  fsD1=fsD2; fsD2=fsD3; fsD3=_ftoInd.FisherSeries[Bars.Range.To-i];
...
// ==========  Статистика ===========================================		
				//if (_frInd.TopSeries[Bars.Range.To-i].Equals(double.NaN) && _frInd.BottomSeries[Bars.Range.To-i].Equals(double.NaN))
				//{ //Print("{0} - InitFractal - {1} - NAN",Bars[Bars.Range.To-i].Time,Instrument.Name); 
				//	}
				//else
				//{ //if (!_frInd.TopSeries[Bars.Range.To-i].Equals(double.NaN)) 
							//Print("{0} - InitFractal - {3} - frU={1} kfu={2}",
				//Bars[Bars.Range.To-i].Time,_frInd.TopSeries[Bars.Range.To-i],kfu,Instrument.Name);
				
				
				//if (!_frInd.BottomSeries[Bars.Range.To-i].Equals(double.NaN)) 
							//Print("{0} - InitFractal - {3} - frD={1} kfd={2}",
				//Bars[Bars.Range.To-i].Time,_frInd.BottomSeries[Bars.Range.To-i],kfd,Instrument.Name);
					
				//}
//===============================================================================				
			  	  i++;
        	}			
					// Print("InitFr {9} - Init OK! -------------- {0} frU1={1} frU2={2} frU3={3} frD1={4} frD2={5} frD3={6}  zmax={7} zmin={8}",
						 ...
						 {    
							...
							{ 	k2++; 
								...
								{	
									...
									if(k2==8) { if(st=="0") torg=false; if(st=="1") torg=true; }
								}
								st="";
							} else { st = st+line[j]; }	
						  }
