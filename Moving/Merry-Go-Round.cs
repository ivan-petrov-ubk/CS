...
{
    [TradeSystem("Merry-Go-Round")]
    public class MerryGoRound : TradeSystem
    {

		[Parameter("Use_Trail", DefaultValue = true)]
...
//		public bool BolaOnly { get;set; }
		...
		[Parameter("Static TP :", DefaultValue = 50, MinValue=10, MaxValue = 5000)]
...
//		[Parameter("Extremum Count:", DefaultValue = 3, MinValue=1, MaxValue = 50)]
//		public int ExtremumCount { get;set; }

//		[Parameter("Mode_Ext", DefaultValue = true)]
//		public bool Mode3b { get;set; }
...
/// </summary>
		...
		[Parameter("Log_Path", DefaultValue = @"MerryGoRoundLoG")]
...
//		private BollingerBands BolaInd;
		...
		private static int magicNumber = 977;
		
		private static int aBuyCount = 0;
		private static int aSellCount = 0;
		...
		protected override void Init()
        {
            ...
            Print("Starting TS on account: {0}, Date: {1}", this.Account.Number, DateTime.Now);
        	if(LogMode) 
			{
				InitLogFile();
			}
#region	GetIndicatorsAndFrames // Get Indicators' and series' references...           
        ...
        protected override void NewBar()
        {
			...
			aBuyCount = aSellCount = 0; 
			if(activePositions!=null)
			{
				for(int ipos = activePositions.Length - 1; ipos >= 0; ipos--)
				{
					if((int)activePositions[ipos].Type == (int)ExecutionRule.Buy)
					{
						aBuyCount++;
					}
					else
			...
			if(listOfBuyStop.Count==0 && aBuyCount==0)
			{
				if(CurrentAsk < (ExtremumMax - 2 * Instrument.Spread))
				{
					setPendingBuyOrder();
				}
			}
			if(listOfSellStop.Count==0 && aSellCount==0)
			{
				if(CurrentBid > (ExtremumMin + 2 * Instrument.Spread))
				{
					setPendingSellOrder();
				}
			}
		...
		protected void setMarketSellOrder()
		{
			...
			double SL = CalculateSL(ExecutionRule.Sell,CurrentBid);
			
			if(SL <= (CurrentBid + 2 * Instrument.Spread))
			{
				SL =  Math.Round(CurrentBid + staticSL * Instrument.Point,Instrument.PriceScale);
			}
			
			double TP = CalculateTP(ExecutionRule.Sell,0);
			
			if( TP >= (CurrentAsk - 2 * Instrument.Spread))
			{
				TP = Math.Round(CurrentAsk - staticTP * Instrument.Point,Instrument.PriceScale);
			}
		...
		protected void TrailPendingOrders()
		{
			...
			if((CurrentAsk == double.NaN) || (CurrentBid == double.NaN)) 
			{
			...
			foreach(Guid pos in listOfBuyStop)
			{
				...
				OP = CalculateOpenPrice(ExecutionRule.BuyStop);
				...
				double SL = CalculateSL(ExecutionRule.BuyStop, OP); //position.StopLoss==null? (OP - staticSL * Instrument.Point): (double)position.StopLoss;
				double TP = Math.Round(CalculateTP(ExecutionRule.BuyStop,OP), Instrument.PriceScale);
				...
				do 
				{
					...
					ntR = Trade.UpdatePendingPosition(	position.Id,
														...
														OP,
														-1,
														position.ExpireTime, //Bars[_CIndex].Time.AddDays(90.0),
														SL,
														TP,
														"buyStop set OP");	
				}
			...
			foreach(Guid pos in listOfSellStop)
			{
				...
				OP = CalculateOpenPrice(ExecutionRule.SellStop);
				...
				double SL = CalculateSL(ExecutionRule.SellStop, OP); //position.StopLoss==null? (OP + staticSL * Instrument.Point): (double) position.StopLoss;
				double TP = Math.Round(CalculateTP(ExecutionRule.SellStop,OP), Instrument.PriceScale);
		...
		protected void CalculateExtremums()
		{
