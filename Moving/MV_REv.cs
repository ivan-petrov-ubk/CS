...
namespace IPro.TradeSystems
{
	...
	public class StrategyMovingAverage : TradeSystem
    {
        ...
        [Parameter("LR_Period", DefaultValue = 10, MinValue = 1, GroupId = __ui_cGroup_Parameters1)]
        ...
        [Parameter("LR_Period", DefaultValue = 20, MinValue = 1, GroupId = __ui_cGroup_Parameters2)]
		...
		protected override void NewQuote()
        {
			...
			if (Instrument.Spread<0.00011) Analyze();
        }
		
		protected override void NewBar()
        {
			//Analyze();
			//Print("---------------------------n {0}",Instrument.Spread);
		}
		...
		private void Analyze()
		{
			...
			if (UseFirstMA && UseSecondMA)
			{
				if (_isMA1BelowMA2 && (_ma1Value>=_ma2Value))
				{
					...
					if (SlowMACrosFastMABottomTop.Equals(AdvisorOrderType.Buy)) Sell(SignalType.SlowMACrosFastMABottomTop);
					if (SlowMACrosFastMABottomTop.Equals(AdvisorOrderType.Sell)) Buy(SignalType.SlowMACrosFastMABottomTop);
				...
				if (!_isMA1BelowMA2 && (_ma1Value<_ma2Value))
				{
					...
					if (SlowMACrosFastMATopBottom.Equals(AdvisorOrderType.Buy)) Sell(SignalType.SlowMACrosFastMATopBottom);
					if (SlowMACrosFastMATopBottom.Equals(AdvisorOrderType.Sell)) Buy(SignalType.SlowMACrosFastMATopBottom);
				}
			}
				...
				if (QuoteAboveMA.Equals(AdvisorOrderType.Buy)) Sell(SignalType.QuoteAboveMA); 
				if (QuoteAboveMA.Equals(AdvisorOrderType.Sell)) Buy(SignalType.QuoteAboveMA); 	
				}
				...
				if (QuoteBelowMA.Equals(AdvisorOrderType.Buy)) Sell(SignalType.QuoteBelowMA); 
				if (QuoteBelowMA.Equals(AdvisorOrderType.Sell)) Buy(SignalType.QuoteBelowMA); 	
				}
		}
