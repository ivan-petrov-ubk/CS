...
//====== START  FIRST  ==============================================================================================				
		    ...
		    {   first=false; 	
				if(tu) { 
					...
					XXPrint("{0} -FIRST- TU зоны nkz4v={1} nkz2v={2}",DTime,nkz4v,nkz2v);
					}				
				...
				if(td) { 		
					 ...
					 nkz4v=Math.Round(nkz4v,Instrument.PriceScale);  
					 nkz2v=Math.Round(nkz2v,Instrument.PriceScale);
					XXPrint("{0} --FIRST- TD зоны nkz4v={1} nkz2v={2}",DTime,nkz4v,nkz2v);
						}
...
//========  END FIRST ============================================================================================================      		
		  if (tu && Bars[ci].High>zmax) 
		  		{ 
			        ...
			        nkz2v=Math.Round(nkz2v,Instrument.PriceScale); 
			 		nd=false; nu=false;
					XXPrint("{0} -------- TU зоны nkz4v={1} nkz2v={2}",DTime,nkz4v,nkz2v);
		  ...
		  if (td && Bars[ci].Low<zmin ) 
		  		{ 
					...
					nd=false; nu=false; 
					XXPrint("{0} -------- TD зоны nkz4v={1} nkz2v={2}",DTime,nkz4v,nkz2v);
				

		 		}
//== Касание зоны/1\2 ======================================================================================================
			if(n2) {
				if (tu && Bars[ci].Low<nkz2v  && !nu)  { nu=true; XXPrint("{0} --------U Касание зоны 1/2",DTime);}
				if (td && Bars[ci].High>nkz2v && !nd)  { nd=true; XXPrint("{0} --------D Касание зоны 1/2",DTime);} 
			}
			else  {
				if (tu && Bars[ci].Low<nkz4v  && !nu)  { nu=true; XXPrint("{0} --------U Касание зоны 1/4",DTime);}
				if (td && Bars[ci].High>nkz4v && !nd)  { nd=true; XXPrint("{0} --------D Касание зоны 1/4",DTime);} 
			}				
				...
				while(!(_ftoInd.FisherSeries[Bars.Range.To-kl7]<0  &&  _ftoInd.FisherSeries[Bars.Range.To-kl7-1]>0) && kl7<1000);
					...
					{ 	if(mui1>mdi1) 
					 ...
					 if(mu1>0 && md1>0 && md2>0) fbD=Math.Round((mu1-md1)/(mu1-md2),2); else fbD=-1;
 						XXPrint("{0}  MIN md1={1} md2={2} md3={3} md4={5} fbD={6}",DTime,md1,md2,md3,md4,fbD);
					}	
			} 
			...
			if( _ftoInd.FisherSeries[Bars.Range.To-2]>0 && _ftoInd.FisherSeries[Bars.Range.To-1]<0 ) // MAX
			    ...
			    if(maxuf>kf1) 
				  {   if(mui1<mdi1)  {
					...
					XXPrint("{0}  MAX mu1={1} mu2={2} mu3={3} mu4={5} fbU={6}",DTime,mu1,mu2,mu3,mu4,fbU);
				  }  
       		 	} 
//=================================================================================================================================== 
