...
{
    [TradeSystem("NF2_v2")]               //copy of "NF2_v1"
...
//================================ Определение патерна   ПИК ПОСЛЕДНЕГО ФРАКТАЛА ПАТЕРНА ВНИЗУ =====================
		   ...
		   {   
           	  if( frU3>frU2 && frU1>frU2  && frU1>0 && frU2>0 && frU3>0) 
				{
					...
					if(UBU>20 && UBU<SL1 && Bars[ci].Close>nkz2 && PCU>kof  && fsU1>0 && fsU2<0 && fsU3<0 && torg) 
					{
...
//=============== Определение патерна   ПИК ПОСЛЕДНЕГО ФРАКТАЛА ПАТЕРНА ВВЕРХУ =====================================
			...
			{         
				...
				{     
					...
					if(UBD>20 && UBD<SL1 && Bars[ci].Close<nkz2 && PCD>kof && fsD1<0 && fsD2>0 && fsD3>0 && torg) 
					{	
						if (posGuidSell!=Guid.Empty && posGuidSell2==Guid.Empty) 
							{   if(TP2!=0) PRDi=PRDi2;
							var res3=Trade.OpenMarketPosition(Instrument.Id, ExecutionRule.Sell, lot,Instrument.Ask, -1,	
								Stops.InPips(UBDi,PRDi), null, null);
