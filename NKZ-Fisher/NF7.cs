...
if(md1>0)  { tD1.Point=new ChartPoint(mdt1, md1); tD1.Caption=string.Format("D1-{0}",fbU); }
				...
				if(tu) { 
				// ======================================  Статистика =================================
				XXPrint("{0} U1 Усл={1} torg={2} nu={3} tu={4} fbU={5} mu1={6} mu2={7} Fish={8}",DTime,
					(torg && nu && tu && fbU>0.5 && fbU<0.8 && mu1>mu2 && _ftoInd.FisherSeries[Bars.Range.To-1]>kf1),
					torg,nu,tu,fbU,mu1,mu2,_ftoInd.FisherSeries[Bars.Range.To-1]);
...
toolTrendLine2.Point1= new ChartPoint(tmax, nkz2v); toolTrendLine2.Point2= new ChartPoint(tmax.AddDays(1), nkz2v);
				...
				if(td) {
				// ======================================  Статистика =================================	
				XXPrint("{0} D1 Усл={1} torg={2} nd={3} td={4} fbD={5} md1={6} md2={7} Fish={8}",DTime,
					(torg && nd && td && fbD>0.4 && fbD<0.8 && md1<md2 && _ftoInd.FisherSeries[Bars.Range.To-1]<-kf1),
					torg,nd,td,fbD,md1,md2,_ftoInd.FisherSeries[Bars.Range.To-1]);					
...
//===================================================================================================================================			
			...
			{ kl7=0;  mind=double.MaxValue; mindf=double.MaxValue; torg=true;
				do 
				{   kl7++;            
...
if(mu4>0  && md4>0) { toolTrendLineD4.Point1= new ChartPoint(mdt4, md4); toolTrendLineD4.Point2= new ChartPoint(mut4, mu4); }	
				...
				// ======================================  Статистика =================================
				XXPrint("{0} U Усл={1} torg={2} nu={3} tu={4} fbU={5} mu1={6} mu2={7} Fish={8}",DTime,
					(torg && nu && tu && fbU>0.5 && fbU<0.8 && mu1>mu2 && _ftoInd.FisherSeries[Bars.Range.To-1]>kf1),
					torg,nu,tu,fbU,mu1,mu2,_ftoInd.FisherSeries[Bars.Range.To-1]);					
			} 
			
			if( _ftoInd.FisherSeries[Bars.Range.To-2]>0 && _ftoInd.FisherSeries[Bars.Range.To-1]<0 ) // MAX - Fisher +/- зверху/вниз
				{ kl7=0;  maxu=double.MinValue; maxuf=double.MinValue; torg=true;
				do
				{ kl7++;      
...
if(mu4>0  && md4>0) { toolTrendLineU4.Point1= new ChartPoint(mut4, mu4); toolTrendLineU4.Point2= new ChartPoint(mdt4, md4); }	
				...
				// ======================================  Статистика =================================	
				XXPrint("{0} D Усл={1} torg={2} nd={3} td={4} fbD={5} md1={6} md2={7} Fish={8}",DTime,
					(torg && nd && td && fbD>0.4 && fbD<0.8 && md1<md2 && _ftoInd.FisherSeries[Bars.Range.To-1]<-kf1),
					torg,nd,td,fbD,md1,md2,_ftoInd.FisherSeries[Bars.Range.To-1]);					  
       		 	} 
//=================================================================================================================================== 
if(torg && nd && td && fbD>0.4 && fbD<0.8 && md1<md2 && _ftoInd.FisherSeries[Bars.Range.To-1]<-kf1)
		{           torg=false;
...
if(torg && nu && tu && fbU>0.5 && fbU<0.8 && mu1>mu2 && _ftoInd.FisherSeries[Bars.Range.To-1]>kf1) 
	    {   torg=false;
...
//====== History =================================================================================================		
 		protected void InitFr()
		{
			...
			if( _ftoInd.FisherSeries[cu-1]>0 && _ftoInd.FisherSeries[cu]<0 ) // MAX
			  ...
			  //Print("{3} -- {0} zmax={1} zmin={2} Volume={4} ", Bars[cu].Time,zmax,zmin,i,Bars[cu].Volume);
