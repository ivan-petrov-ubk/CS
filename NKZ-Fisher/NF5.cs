...
//====== START  FIRST  ==============================================================================================				
			if(first) 
		    {   first=false; k7=0; mind=double.MaxValue; mindf=double.MaxValue; mu1=0; md2=0; mu1=0;mu2=0;
				 kt=0; 			   maxu=double.MinValue; maxuf=double.MinValue;
			do 
			{  k7++;  
				
				if(_ftoInd.FisherSeries[Bars.Range.To-k7]<0) {
					if(Bars[Bars.Range.To-k7].Low<mind) { mind=Bars[Bars.Range.To-k7].Low; 
					mindt=Bars[Bars.Range.To-k7].Time; mindi=Bars.Range.To-k7; }
					if(_ftoInd.FisherSeries[Bars.Range.To-k7]<mindf) mindf=_ftoInd.FisherSeries[Bars.Range.To-k7]; }
				
				if(_ftoInd.FisherSeries[Bars.Range.To-k7]>0) {
					if(Bars[Bars.Range.To-k7].High>maxu) { maxu=Bars[Bars.Range.To-k7].High; 
					maxut=Bars[Bars.Range.To-k7].Time; maxui=Bars.Range.To-k7; }  
					if(_ftoInd.FisherSeries[Bars.Range.To-k7]>maxuf) maxuf=_ftoInd.FisherSeries[Bars.Range.To-k7];  }
					
				// MAX
				if (_ftoInd.FisherSeries[Bars.Range.To-k7]>0  &&  _ftoInd.FisherSeries[Bars.Range.To-k7-1]<0)	{
					Print("MAX -- kt={0} k7={1} maxuf={2} mu1={3} mu2={4}",kt,k7,maxuf,mu1,mu2);
					if(maxuf>kf) {
					if (kt>0) {	
						 if(mu1==0) {mu1=maxu; mut1=maxut; mui1=maxui;} else { 
						 if(maxu>mu1 && mui1<mdi1)  { mu1=maxu; mut1=maxut; mui1=maxui; } }
						 if(mu1!=0 && mdi1<mui1)    { mu2=maxu; mut2=maxut; mui2=maxui; }
						
						if(mu1==0) { mu1=maxu; mut1=maxut; mui1=maxui; } else {
						if(md1==0) { mu1=maxu; mut1=maxut; mui1=maxui; } else {
						if(mu2==0) { mu2=maxu; mut2=maxut; mui2=maxui;} else }
					
					} }
					mind=double.MaxValue; mindf=double.MaxValue;
					kt++; } }
				// MIN
				if (_ftoInd.FisherSeries[Bars.Range.To-k7]<0  &&  _ftoInd.FisherSeries[Bars.Range.To-k7-1]>0)	{
					Print("MIN -- kt={0} k7={1} mindf={2} md1={3} md2={4}",kt,k7,mindf,md1,md2);
					if(mindf<-kf) {
					if (kt>0) {	
						if(md1==0 && mu1==0) { md1=mind; mdt1=mindt; mdi1=mindi; }
						
						
						if(md1==0) { md1=mind; mdt1=mindt; mdi1=mindi; } else {if(md2==0) {md2=mind; mdt2=mindt; mdi2=mindi;} } }
					maxu=double.MinValue; maxuf=double.MinValue;
					kt++; }	}
				
			} while((mu2==0 || md2==0) && k7<500);
			
			  Print("START kt={0} k7={1} ===========================================",kt,k7);
			 Print("START 1 --- mu1={0} mu2={1} md1={2} md2={3}",mu1,mu2,md1,md2);
			 Print("START 2 --- mut1={0} mut2={1} mdt1={2} mdt2={3}",mut1,mut2,mdt1,mdt2);
			 Print("START 3 --- mui1={0} mui2={1} mdi1={2} mdi2={3}",mui1,mui2,mdi1,mdi2);
				if(tu) { 
