...
{
    [TradeSystem("NF3")]                //copy of "NF2_v2"
    public class ZZ_Ex1 : TradeSystem
    {
        ...
        {	nu=false; nd=false; md=0; mu=0;
			...
			if(ver>1) { ReDat(); Print("ReDat-  mu2={0}-{1}  md2={2}-{3}  mu1={4}-{5}  md1={6}-{7} Range={8}",mu2,mui2,md2,mdi2,mu1,mui1,md1,mdi1,Bars.Range.To-1); 
								var mt=Bars.Range.To-mui2;
								Print("{0} - {1} - {2} - {3}",Bars[Bars.Range.To-mt].Time,Bars[mdi2].Time,Bars[mui1].Time,Bars[mdi1].Time);
			}
			...
			{ kl7=0;  mind=double.MaxValue; mindf=double.MaxValue; 
				...
				while(!(_ftoInd.FisherSeries[Bars.Range.To-kl7]<0  &&  _ftoInd.FisherSeries[Bars.Range.To-kl7-1]>0) && kl7<1000);
					...
					{ 	if(mui1>mdi1) 
					 ...
					 XPrint("{0};{1};{2};{3};{4};{5};{6};{7};{8};",DTime,mu2,mui2,md2,mdi2,mu1,mui1,md1,mdi1);			 
...
// END InitFile()  ===============================================================================		  
//===============  Read Date   ===============================================================================		
 		protected void ReDat()
		{ 
			trueBuyPath=PathToLogFile+"\\"+Instrument.Name.ToString()+".DAT"; 
			LogRD = new StreamReader(File.Open(trueBuyPath, FileMode.Open,FileAccess.Read,FileShare.Read));
		    if(LogRD!=null)
		    {   st="";
 		       string line;
  			      while ((line = LogRD.ReadLine()) != null)
    				    { 
       				     k2=0; name2="";
						for (int j = 0; j < line.Length; j++)
						 {    
							...
							{ 	k2++; 
								//XPrint("{0};{1};{2};{3};{4};{5};{6};{7};{8};",DTime,mu2,mui2,md2,mdi2,mu1,mui1,md1,mdi1);					
									...
									if(k2==2)  mu2=Convert.ToDouble(st);
									if(k2==3)  { mui2=Convert.ToInt32(st); mut2=Bars[mui2].Time; }
									
									if(k2==4)  md2=Convert.ToDouble(st);
									if(k2==5)  { mdi2=Convert.ToInt32(st); mdt2=Bars[mdi2].Time; }
									
									if(k2==6)  mu1=Convert.ToDouble(st); 
									if(k2==7)  { mui1=Convert.ToInt32(st); mut1=Bars[mui1].Time; }
									
									if(k2==8) md1=Convert.ToDouble(st); 
									if(k2==9)  { mdi1=Convert.ToInt32(st); mdt1=Bars[mdi1].Time; }
