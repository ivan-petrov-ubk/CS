...
namespace IPro.TradeSystems
{
    ...
    public class ZZ_Ex1 : TradeSystem
    {
		...
		public bool frU,frD,first=true;
...
//====== START  FIRST  ==============================================================================================				
		    ...
		    {   first=false;
//========== История ================================================================================================
				kl7=0; kt=0; kd=0; ku=0; maxu=double.MinValue; maxuf=double.MinValue;
				 		      mind=double.MaxValue; mindf=double.MaxValue; 
				do
				{ kl7++; 
					//Print("Start 1 --- {0}",Bars[Bars.Range.To-kl7].Time);
					...
					Print("Start mind={0} Time={1} mindt={2} mindi={3}",mind,Bars[Bars.Range.To-kl7].Time,mindt,mindi);
					}
					if(_ftoInd.FisherSeries[Bars.Range.To-kl7]<mindf) { mindf=_ftoInd.FisherSeries[Bars.Range.To-kl7];
					Print("Start mindf = {0}",mindf); }
					//Print("Start 2 --- {0}",Bars[Bars.Range.To-kl7].Low);
					if(Bars[Bars.Range.To-kl7].High>maxu) {maxu=Bars[Bars.Range.To-kl7].High; 
						maxut=Bars[Bars.Range.To-kl7].Time; maxui=Bars.Range.To-kl7;
					Print("Start maxu={0} Time={1} maxut={2} maxui={3}",maxu,Bars[Bars.Range.To-kl7].Time,maxut,maxui);
					}  
					if(_ftoInd.FisherSeries[Bars.Range.To-kl7]>maxuf) { maxuf=_ftoInd.FisherSeries[Bars.Range.To-kl7];	
					Print("Start maxuf = {0}",maxuf);}
					// MIN
					if(_ftoInd.FisherSeries[Bars.Range.To-kl7]<0  &&  _ftoInd.FisherSeries[Bars.Range.To-kl7-1]>0) 
					{// Print("MIN-----------------------------------------------");
						if(kt>0) {
							if(mindf<-kf1) 
							{ 	kd++;
								if(kd==1) { md1=mind; mdt1=mindt; mdi1=mindi; 
								var vl=Tools.Create<VerticalLine>();vl.Time = mdt1;} 
								if(kd==2) { md2=mind; mdt2=mindt; mdi2=mindi; 
								var vl=Tools.Create<VerticalLine>();vl.Time = mdt2;}
									
							}  }

						maxu=double.MinValue; maxuf=double.MinValue;
				 		mind=double.MaxValue; mindf=double.MaxValue; 
						Print("MIN - {0} -- md1={1} mdt1={2} md2={3} mdt2={4} kt={5} kl7={6}",Bars[Bars.Range.To-kl7].Time,md1,mdt1,md2,mdt2,kt,kl7);
						kt++;
					}
					//MAX
					//Print("MAX ------------------------------------------------ {0}",Bars[Bars.Range.To-kl7].Time);
					if(_ftoInd.FisherSeries[Bars.Range.To-kl7]>0  &&  _ftoInd.FisherSeries[Bars.Range.To-kl7-1]<0) 
					{   //Print("MAX ------------------------------------------------ {0}",Bars[Bars.Range.To-kl7].Time);
						if(kt>0) {
							if(maxuf>kf1) 
							{ 	ku++;
								if(ku==1) { mu1=mind; mut1=maxut; mui1=maxui; 
								var vl=Tools.Create<VerticalLine>();vl.Time = mut1; } 
								if(ku==2) { mu2=mind; mut2=maxut; mui2=maxui; 
								var vl=Tools.Create<VerticalLine>();vl.Time = mut2;}
									
							}  }
						maxu=double.MinValue; maxuf=double.MinValue;
				 		mind=double.MaxValue; mindf=double.MaxValue; 
						Print("MAX - {0} -- mu1={1} mut1={2} mu2={3} mut2={4} kt={5} kl7={6}",Bars[Bars.Range.To-kl7].Time,mu1,mut1,mu2,mut2,kt,kl7);
						kt++;
					}
				}				
				while(kt<6 && kl7<1000);
