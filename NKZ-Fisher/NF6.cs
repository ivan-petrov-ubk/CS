...
{
    [TradeSystem("NF6")]                   //copy of "NF5"
    public class ZZ_Ex1 : TradeSystem
    {
		[Parameter("Номер версии NF5", DefaultValue = 1)]
		...
		public int vd1,vd2,vd3,vd4;
		public int vu1,vu2,vu3,vu4,maxv;		
		public int sd1,sd2,sd3,sd4;
		public int su1,su2,su3,su4,sv;				
...
toolTrendLine4.Point1= new ChartPoint(tmax, nkz4v); toolTrendLine4.Point2= new ChartPoint(tmax.AddDays(1), nkz4v);	
toolText1.Point=new ChartPoint(tmax, nkz2v+Instrument.Spread);toolText1.Caption=string.Format("{0}",nkz2v);					
toolTrendLine2.Point1= new ChartPoint(tmax, nkz2v); toolTrendLine2.Point2= new ChartPoint(tmax.AddDays(1), nkz2v);
						}
...
toolText.Point=new ChartPoint(tmin, nkz4v+Instrument.Spread);toolText.Caption=string.Format("{0}",nkz4v);						
toolTrendLine4.Point1= new ChartPoint(tmin, nkz4v); toolTrendLine4.Point2= new ChartPoint(tmin.AddDays(3), nkz4v);
toolText1.Point=new ChartPoint(tmin, nkz2v+Instrument.Spread);toolText1.Caption=string.Format("{0}",nkz2v);	
...
//===================================================================================================================================			
			...
			{ kl7=0;  mind=double.MaxValue; mindf=double.MaxValue; maxv=0;sv=0;
				do 
				{   kl7++; sv=sv+Bars[Bars.Range.To-kl7].Volume;            
					...
					if(Bars[Bars.Range.To-kl7].Volume>maxv) {maxv=Bars[Bars.Range.To-kl7].Volume; maxvt=Bars[Bars.Range.To-kl7].Time; }
...
if(mu4>0  && md4>0) { toolTrendLineD4.Point1= new ChartPoint(mdt4, md4); toolTrendLineD4.Point2= new ChartPoint(mut4, mu4); }	
						...
						if(mu1>0)  { tU1.Point=new ChartPoint(mut1, mu1+Instrument.Spread*2); tU1.Caption=string.Format("U1-{0}",su1); }
						if(mu2>0)  { tU2.Point=new ChartPoint(mut2, mu2+Instrument.Spread*2); tU2.Caption=string.Format("U2-{0}",su2); }
						if(mu3>0)  { tU3.Point=new ChartPoint(mut3, mu3+Instrument.Spread*2); tU3.Caption=string.Format("U3-{0}",su3); }
						if(mu4>0)  { tU4.Point=new ChartPoint(mut4, mu4+Instrument.Spread*2); tU4.Caption=string.Format("U4-{0}",su4); }
						if(md1>0)  { tD1.Point=new ChartPoint(mdt1, md1); tD1.Caption=string.Format("D1-{0}-{1}",fbD,sd1); }
						if(md2>0)  { tD2.Point=new ChartPoint(mdt2, md2); tD2.Caption=string.Format("D2-{0}",sd2); }
						if(md3>0)  { tD3.Point=new ChartPoint(mdt3, md3); tD3.Caption=string.Format("D3-{0}",sd3); }
						if(md4>0)  { tD4.Point=new ChartPoint(mdt4, md4); tD4.Caption=string.Format("D4-{0}",sd4); }
					}	
			} 
			
			if( _ftoInd.FisherSeries[Bars.Range.To-2]>0 && _ftoInd.FisherSeries[Bars.Range.To-1]<0 ) // MAX
				{ kl7=0;  maxu=double.MinValue; maxuf=double.MinValue;maxv=0;sv=0;
				do
				{ kl7++; sv=sv+Bars[Bars.Range.To-kl7].Volume;       
...
if(mu4>0  && md4>0) { toolTrendLineU4.Point1= new ChartPoint(mut4, mu4); toolTrendLineU4.Point2= new ChartPoint(mdt4, md4); }	
						...
						if(mu1>0)  { tU1.Point=new ChartPoint(mut1, mu1+Instrument.Spread*2); tU1.Caption=string.Format("U1-{0}-{1}",fbU,su1); }
						if(mu2>0)  { tU2.Point=new ChartPoint(mut2, mu2+Instrument.Spread*2); tU2.Caption=string.Format("U2-{0}",su2); }
						if(mu3>0)  { tU3.Point=new ChartPoint(mut3, mu3+Instrument.Spread*2); tU3.Caption=string.Format("U3-{0}",su3); }
						if(mu4>0)  { tU4.Point=new ChartPoint(mut4, mu4+Instrument.Spread*2); tU4.Caption=string.Format("U4-{0}",su4); }
						if(md1>0)  { tD1.Point=new ChartPoint(mdt1, md1); tD1.Caption=string.Format("D1-{0}",sd1); }
						if(md2>0)  { tD2.Point=new ChartPoint(mdt2, md2); tD2.Caption=string.Format("D2-{0}",sd2); }
						if(md3>0)  { tD3.Point=new ChartPoint(mdt3, md3); tD3.Caption=string.Format("D3-{0}",sd3); }
						if(md4>0)  { tD4.Point=new ChartPoint(mdt4, md4); tD4.Caption=string.Format("D4-{0}",sd4); }
				  }  
...
//=================================================================================================================================== 
if(nd && td && fbU>0.4 && fbU<0.8 && md1<md2 && _ftoInd.FisherSeries[Bars.Range.To-1]<-kf1)
...
if(nu && tu && fbD>0.5 && fbD<0.8 && mu1>mu2 && _ftoInd.FisherSeries[Bars.Range.To-1]>kf1) 
	    { 
...
//====== History =================================================================================================		
		...
		{ 
			...
			Print("INIT-DW-{0};{1};{2};{3};{4};{5};{6};{7};{8};",MTime,mu2,mui2,md2,mdi2,mu1,mui1,md1,mdi1);	
if(mu1>0  && md1>0) { toolTrendLineD1.Point1= new ChartPoint(mdt1, md1); toolTrendLineD1.Point2= new ChartPoint(mut1, mu1); }
if(mu1>0  && md2>0) { toolTrendLineU1.Point1= new ChartPoint(mut1, mu1); toolTrendLineU1.Point2= new ChartPoint(mdt2, md2); } 
if(mu2>0  && md2>0) { toolTrendLineD2.Point1= new ChartPoint(mdt2, md2); toolTrendLineD2.Point2= new ChartPoint(mut2, mu2); }
if(mu2>0  && md3>0) { toolTrendLineU2.Point1= new ChartPoint(mut2, mu2); toolTrendLineU2.Point2= new ChartPoint(mdt3, md3); }
...
if(mu4>0  && md4>0) { toolTrendLineD4.Point1= new ChartPoint(mdt4, md4); toolTrendLineD4.Point2= new ChartPoint(mut4, mu4); }							 
			...
			if( _ftoInd.FisherSeries[cu-1]>0 && _ftoInd.FisherSeries[cu]<0 ) // MAX
				{ kl7=0;  maxu=double.MinValue; maxuf=double.MinValue; sv=0;maxv=0;
				do
				{ kl7++; sv=sv+Bars[cu-kl7].Volume;  
			...
			Print("INIT-UP-{0};{1};{2};{3};{4};{5};{6};{7};{8};",MTime,mu2,mui2,md2,mdi2,mu1,mui1,md1,mdi1);
if(mu1>0  && md1>0) { toolTrendLineU1.Point1= new ChartPoint(mut1, mu1); toolTrendLineU1.Point2= new ChartPoint(mdt1, md1); }				  
if(mu2>0  && md1>0) { toolTrendLineD1.Point1= new ChartPoint(mdt1, md1); toolTrendLineD1.Point2= new ChartPoint(mut2, mu2); } 
if(mu2>0  && md2>0) { toolTrendLineU2.Point1= new ChartPoint(mut2, mu2); toolTrendLineU2.Point2= new ChartPoint(mdt2, md2); }
if(mu3>0  && md2>0) { toolTrendLineD2.Point1= new ChartPoint(mdt2, md2); toolTrendLineD2.Point2= new ChartPoint(mut3, mu3); }
...
if(mu4>0  && md4>0) { toolTrendLineU4.Point1= new ChartPoint(mut4, mu4); toolTrendLineU4.Point2= new ChartPoint(mdt4, md4); }	
			  ...
			  if(Bars[cu].High>zmax) { zmax=Bars[cu].High; tmax=Bars[cu].Time; }
			  if(Bars[cu].Low<zmin)  { zmin=Bars[cu].Low;  tmin=Bars[cu].Time;  }
			  
			  Print("{3} -- {0} zmax={1} zmin={2} Volume={4} sv={5}", Bars[cu].Time,zmax,zmin,i,Bars[cu].Volume,sv);
		  	 
        	}			
						if(mu1>0)  { tU1.Point=new ChartPoint(mut1, mu1+Instrument.Spread*3); tU1.Caption=string.Format("U1-{0}",su1); }
						if(mu2>0)  { tU2.Point=new ChartPoint(mut2, mu2+Instrument.Spread*3); tU2.Caption=string.Format("U2-{0}",su2); }
						if(mu3>0)  { tU3.Point=new ChartPoint(mut3, mu3+Instrument.Spread*3); tU3.Caption=string.Format("U3-{0}",su3); }
						if(mu4>0)  { tU4.Point=new ChartPoint(mut4, mu4+Instrument.Spread*3); tU4.Caption=string.Format("U4-{0}",su4); }
						if(md1>0)  { tD1.Point=new ChartPoint(mdt1, md1); tD1.Caption=string.Format("D1-{0}",sd1); }
						if(md2>0)  { tD2.Point=new ChartPoint(mdt2, md2); tD2.Caption=string.Format("D2-{0}",sd2); }
						if(md3>0)  { tD3.Point=new ChartPoint(mdt3, md3); tD3.Caption=string.Format("D3-{0}",sd3); }
						if(md4>0)  { tD4.Point=new ChartPoint(mdt4, md4); tD4.Caption=string.Format("D4-{0}",sd4); }			
...
//== Log  File ===================================================================================================   
		...
		{       var trueLogPath = PathToLogFile+"\\"+Instrument.Name.ToString()+".LOG";
