...
{
    [TradeSystem("MAX1")] //copy of "Elder"
		...
		if ( Bars[Bars.Range.To-1].Time.Hour>=tm2 ) 
		  {  
			...
			{  //Buy
			    ...
			    if (posGuidBuy==Guid.Empty) { ib=true; kl=0; var result1 = Trade.OpenMarketPosition(Instrument.Id, ExecutionRule.Buy, 0.1,Instrument.Bid, -1,Stops.InPips(300,100), null, null);
											Print("BUY   {0} - {1} ",Bars[Bars.Range.To-1].Time,db);					      
											if (result1.IsSuccessful)  posGuidBuy=result1.Position.Id; } 				
			}
			
			if(dmv<0 || dmacd<0) 
			{ ib=false;	if (posGuidBuy!=Guid.Empty && Trade.GetPosition(posGuidBuy).State==PositionState.Active) 
									{var res = Trade.CloseMarketPosition(posGuidBuy); if (res.IsSuccessful) posGuidBuy = Guid.Empty;}		        
			}
			
			if(kl==5) 
			{	kl=100; if (posGuidBuy!=Guid.Empty && Trade.GetPosition(posGuidBuy).State==PositionState.Active) 
									{var res = Trade.CloseMarketPosition(posGuidBuy); if (res.IsSuccessful) posGuidBuy = Guid.Empty;}	
				if (posGuidSell!=Guid.Empty && Trade.GetPosition(posGuidSell).State==PositionState.Active) 
									{var res = Trade.CloseMarketPosition(posGuidSell); if (res.IsSuccessful) posGuidSell = Guid.Empty;}						
			}
			
			if(torg && dmv<0 && dmacd<0 && !isl) 
			{ //Sell
				...
				if (posGuidSell==Guid.Empty) {isl=true; kl=0;	var result2 = Trade.OpenMarketPosition(Instrument.Id, ExecutionRule.Sell, 0.1,Instrument.Ask, -1,Stops.InPips(300,100), null, null);
											Print("SELL  {0} - {1} ",Bars[Bars.Range.To-1].Time,db);
												if (result2.IsSuccessful)  posGuidSell=result2.Position.Id;	} 				
			}
			
			if(dmv>0 || dmacd>0) { isl=false; if (posGuidSell!=Guid.Empty && Trade.GetPosition(posGuidSell).State==PositionState.Active) 
											{kl=100; var res = Trade.CloseMarketPosition(posGuidSell); if (res.IsSuccessful) posGuidSell = Guid.Empty;}	
			}	
