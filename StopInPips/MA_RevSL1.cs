...
{
    [TradeSystem("MA_RevSL1")]  //copy of "MA_Rev1"
		...
		private void Analyze()
		{
			...
			if (UseFirstMA && UseSecondMA)
			{
				if (_isMA1BelowMA2 && (_ma1Value>=_ma2Value))
				{
					...
					if (SlowMACrosFastMABottomTop.Equals(AdvisorOrderType.Buy) && !pMA) Sell(SignalType.SlowMACrosFastMABottomTop);
					if (SlowMACrosFastMABottomTop.Equals(AdvisorOrderType.Sell) && pMA) Buy(SignalType.SlowMACrosFastMABottomTop);
				...
				if (!_isMA1BelowMA2 && (_ma1Value<_ma2Value))
				{
					...
					if (SlowMACrosFastMATopBottom.Equals(AdvisorOrderType.Buy) && !pMA)  Sell(SignalType.SlowMACrosFastMATopBottom);
					if (SlowMACrosFastMATopBottom.Equals(AdvisorOrderType.Sell) && pMA) Buy(SignalType.SlowMACrosFastMATopBottom);
				}
			}
				...
				if (QuoteAboveMA.Equals(AdvisorOrderType.Buy) && !pMA) Sell(SignalType.QuoteAboveMA); 
				if (QuoteAboveMA.Equals(AdvisorOrderType.Sell) && pMA) Buy(SignalType.QuoteAboveMA); 	
				}
...
//			var result=Trade.Buy(Instrument.Id, LotVolume, Stops.InPips(StopLoss, TakeProfit)); 	
			var result=Trade.Buy(Instrument.Id, LotVolume, Stops.InPips(200, 20)); 			
...
//			var result=Trade.Sell(Instrument.Id, LotVolume, Stops.InPips(StopLoss, TakeProfit)); 
			var result=Trade.Sell(Instrument.Id, LotVolume, Stops.InPips(200, 20)); 
