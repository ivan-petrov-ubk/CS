...
{
    [TradeSystem("TC_Revers_ZigZag")]  //copy of "TC_Revers7"
    public class TC_Revers4 : TradeSystem
    {
		...
		[Parameter("Use_Buy)", DefaultValue = true)]
        public bool BuyOn;
		[Parameter("Use_Sell)", DefaultValue = true)]
        public bool SellOn;
		...
		double aGuba5, aZub5, aChelust5,ZZ5;   // Челюсть Синяя
...
//=== КОРЕКЦИЯ ===========================================================================================							 
			...
			if ( DTime.Hour==4 && DTime.Minute==0 ) Torg=true;
			if ( DTime.Hour==8 && DTime.Minute==0 ) Torg=false;
//====  BUYLIMIT  START  =====================================================================================================
			if(Torg && frDown  && fr_all_Down_L && posGuidBuy==Guid.Empty ) 
			{
				        // Открываем BuyLimit
				var result = Trade.OpenPendingPosition(Instrument.Id, ExecutionRule.BuyLimit, 0.1, fr_all_Down-TP, 0, Stops.InPips(250,null), null, null, null);
						 if (result.IsSuccessful) { posGuidBuy=result.Position.Id;  
...
//==   SELLIMIT START   ================================================================================						  
			if(Torg && frUp  &&   fr_all_Up_L && posGuidSell==Guid.Empty )
			{ 
			    var result = Trade.OpenPendingPosition(Instrument.Id, ExecutionRule.SellLimit, 0.1, fr_all_Up+TP, 0, Stops.InPips(250,null), null, null, null);
