...
{
    [TradeSystem("ZZT")]    //copy of "ZZ_Ex4_Fractal_Test"
...
//======================================================================================================================================		  
		    ...
		    if ( _ftoInd.FisherSeries[Bars.Range.To-2]<0.3  &&  _ftoInd.FisherSeries[Bars.Range.To-1]>0.3) {FU=Bars.Range.To; isFU=true;} else isFU=false;
			if ( _ftoInd.FisherSeries[Bars.Range.To-2]>-0.3  &&  _ftoInd.FisherSeries[Bars.Range.To-1]<-0.3) {FD=Bars.Range.To; isFD=true;} else isFD=false;
...
//======================================================================================================================================
			...
			{  
				...
				{ // ВВЕРХУ
					...
					if(sl && zzd1>FD && zzd3<=FD && zzd3>FU && zzu1<zzu3 && zzu1>zzu2+(zzu3-zzu2)*0.3 && zzu1<zzu2+(zzu3-zzu2)*0.9)	
						{
						 var vr=Tools.Create<VerticalLine>(); vr.Color=Color.Red; vr.Time=Bars[zzd2].Time; vr.Width=3;
						 if(posGuidSell==Guid.Empty){
							var result1 = Trade.OpenPendingPosition(Instrument.Id, ExecutionRule.SellStop, 1.0,  zzu2-0.0003, 0, Stops.InPips(300,600), null, null, null);
						      if (result1.IsSuccessful) posGuidSell=result1.Position.Id; else 
						      {
								var result2 = Trade.OpenMarketPosition(Instrument.Id, ExecutionRule.Sell, 1.0,Instrument.Ask, -1,Stops.InPips(300,600), null, null);
				...
				{ // ВНИЗУ
					...
					if(bu && zzd1>FU && zzd3<=FU && zzd3>FD && zzu1<zzu2-(zzu2-zzu3)*0.3 && zzu1>zzu2-(zzu2-zzu3)*0.9)	
					
					{var vr=Tools.Create<VerticalLine>(); vr.Color=Color.Blue; vr.Time=Bars[zzd2].Time; vr.Width=3;
						if(posGuidBuy==Guid.Empty){  	
						var result = Trade.OpenPendingPosition(Instrument.Id, ExecutionRule.BuyStop, 1.0, zzu2+0.0003, 0, Stops.InPips(200,600), null, null, null);	
						     if (result.IsSuccessful)  posGuidBuy=result.Position.Id; else 
						     {
								var result3 = Trade.OpenMarketPosition(Instrument.Id, ExecutionRule.Buy, 1.0,Instrument.Bid, -1, Stops.InPips(200,600), null, null);
