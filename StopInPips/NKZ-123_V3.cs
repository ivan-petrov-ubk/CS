...
// Первое начертание зон	=====================================================================================		
		  ...
		  { 
			...
			if(tu) { zmax = Bars[iFT].High;  
				 	...
				 	XXPrint("Первая прорисовка ВВЕРХ - {0} - nkz2={1} nkz4={2} - 1U",Bars[Bars.Range.To-1].Time,nkz2,nkz4);
                   }
			...
			if(td) { zmin = Bars[iFT].Low;
					 ...
					 nkz4v=Math.Round(nkz4v,Instrument.PriceScale);  
					 nkz2v=Math.Round(nkz2v,Instrument.PriceScale);  
				 	XXPrint("Первая прорисовка ВНИЗ - {0} - nkz2={1} nkz4={2} - 1D",Bars[Bars.Range.To-1].Time,nkz2,nkz4);
  				   }
...
//====================================================================================================================      		
	  if(k>1) {
		  if (tu && Bars[Bars.Range.To-1].High>zmax) { 
			         ...
			         nkz4v=Math.Round(nkz4v,Instrument.PriceScale);  
			         nkz2v=Math.Round(nkz2v,Instrument.PriceScale); 
 			XXPrint("ПЕРЕРИСОВКА ВВЕРХ - {0} - nkz2={1} nkz4={2} - 1U",Bars[Bars.Range.To-1].Time,nkz2,nkz4);
		  ...
		  if (td && Bars[Bars.Range.To-1].Low<zmin ) { 
			        ...
			        nkz4v=Math.Round(nkz4v,Instrument.PriceScale);  
			        nkz2v=Math.Round(nkz2v,Instrument.PriceScale); 
 			XXPrint("ПЕРЕРИСОВКА ВНИЗ - {0} - nkz2={1} nkz4={2} - 1U",Bars[Bars.Range.To-1].Time,nkz2,nkz4);
...
//== Касание зоны/1\4 ======================================================================================================
		  if(n4) {
			if (tu && Bars[ci].Close<nkz4 && !nu)  { nu=true; XXPrint("ПЕРЕСЕЧЕНИЕ 1/4 ВНИЗ - {0}",Bars[Bars.Range.To-1].Time); }
			if (td && Bars[ci].Close>nkz4 && !nd) { nd=true;  XXPrint("ПЕРЕСЕЧЕНИЕ 1/4 ВВЕРХ - {0}",Bars[Bars.Range.To-1].Time); }
                  } 
			if (tu && Bars[ci].Close<nkz2 && !nu)  { nu=true;  XXPrint("ПЕРЕСЕЧЕНИЕ 1/2 ВНИЗ - {0}",Bars[Bars.Range.To-1].Time);}
			if (td && Bars[ci].Close>nkz2 && !nd) { nd=true;   XXPrint("ПЕРЕСЕЧЕНИЕ 1/2 ВВЕРХ - {0}",Bars[Bars.Range.To-1].Time);}
...
// Определение патерна   ПИК ПОСЛЕДНЕГО ФРАКТАЛА ПАТЕРНА ВНИЗУ 
		   ...
		   { 
           		if( frU3>frU2 && frU1>frU2 ) {
					...
					PRUi=(int)PRU;
					UBUi=(int)UBU;
					XXPrint("PATERN BUY -- {0} -- TP={1} SL={2} TP/SL={3} ",Bars[Bars.Range.To-1].Time,PRUi,UBUi,PCU);
					if(UBU>50 && UBU<200 && PCU>2 && Bars[Bars.Range.To-1].Close>nkz2) {
				    XXPrint("BUY -- {0} -- TP={1} SL={2} TP/SL={3} ",Bars[Bars.Range.To-1].Time,PRUi,UBUi,PCU);
			...
			{ 
				...
				{ 
					...
					XXPrint("PATERN SELL -- {0} -- TP(п)={1} SL(p)={2} TP/SL={3}",Bars[Bars.Range.To-1].Time,PRDi,UBDi,PCD);				
					if(UBD>50 && UBD<200 && PCD>2 && Bars[Bars.Range.To-1].Close<nkz2) {
					XXPrint("SELL -- {0} -- TP={1} SL={2} TP/SL={3} ",Bars[Bars.Range.To-1].Time,PRDi,UBDi,PCD);
