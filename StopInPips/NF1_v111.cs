...
namespace IPro.TradeSystems
{
    ...
    public class ZZ_Ex1 : TradeSystem
    {
		...
		[Parameter("Init_FileName", DefaultValue = @"nkz")]
		public string BuyFileName { get;set; }
		
		private bool tu,td,nu,nd,n2;
		private int NKZ,i,SL1,dl,frac,kof;
	    ...
	    private double dlt,frUp,frDown,lot;
...
//====  Fractal вверх ================================================================================================================
	  if (_frInd.TopSeries[Bars.Range.To-frac]>0) 		
		{  
			 ...
			 Print("Top {0} {1}",Bars[Bars.Range.To-frac].Time,_frInd.TopSeries[Bars.Range.To-frac]);			   
		 }
//====  Fractal вниз ====================================================================================			  
		if (_frInd.BottomSeries[Bars.Range.To-frac]>0)   
		  {  
				  ...
				  } else 
				  {
				   ...
				   Print("Bootom {0} {1}",Bars[Bars.Range.To-frac].Time,_frInd.BottomSeries[Bars.Range.To-frac]);
		   } 			
//====================================================================================================================      		
...
//================================ Определение патерна   ПИК ПОСЛЕДНЕГО ФРАКТАЛА ПАТЕРНА ВНИЗУ =====================
		   if (_frInd.TopSeries[Bars.Range.To-frac]>0)    
		   {   Print("PATERN Top {0} {1}",Bars[Bars.Range.To-frac].Time,_frInd.TopSeries[Bars.Range.To-frac]);
           	  if( frU3>frU2 && frU1>frU2  && frU1>0 && frU2>0 && frU3>0) 
				{
					...
					UBUi=(int)UBU;
					//UBUi=(int)SL1;
					if(UBU>40 && UBU<SL1 && Bars[ci].Close>nkz2 && PCU>kof  && fsU1>0 && fsU2<0) 
					{
...
//=============== Определение патерна   ПИК ПОСЛЕДНЕГО ФРАКТАЛА ПАТЕРНА ВВЕРХУ =====================================
 			if (_frInd.BottomSeries[Bars.Range.To-frac]>0)	   
			{    Print("PATERN Bootom {0} {1}",Bars[Bars.Range.To-frac].Time,_frInd.BottomSeries[Bars.Range.To-frac]);         
				...
				{   TPD=Math.Round(zmin, Instrument.PriceScale);
					...
					if(UBD>40 && UBD<SL1 && Bars[ci].Close<nkz2 && PCD>kof && fsD1<0 && fsD2>0) 
					{	
...
//========== STOP ==========================================
   if (posGuidSell!=Guid.Empty && Trade.GetPosition(posGuidSell).Pips>SL1) 	
	{var res5 = Trade.UpdateMarketPosition(posGuidSell,Trade.GetPosition(posGuidSell).OpenPrice-Instrument.Spread-dlt,Trade.GetPosition(posGuidSell).TakeProfit, null); 
		    if (res5.IsSuccessful) posGuidSell=res5.Position.Id;}
	
   if (posGuidBuy!=Guid.Empty && Trade.GetPosition(posGuidBuy).Pips>SL1) 	
	{var res6 = Trade.UpdateMarketPosition(posGuidBuy,Trade.GetPosition(posGuidBuy).OpenPrice+Instrument.Spread+dlt,Trade.GetPosition(posGuidBuy).TakeProfit, null); 
		    if (res6.IsSuccessful) posGuidBuy=res6.Position.Id;}
	
   if (posGuidSell2!=Guid.Empty && Trade.GetPosition(posGuidSell2).Pips>SL1) 	
	{var res7 = Trade.UpdateMarketPosition(posGuidSell2,Trade.GetPosition(posGuidSell2).OpenPrice-Instrument.Spread-dlt,Trade.GetPosition(posGuidSell2).TakeProfit, null); 
		    if (res7.IsSuccessful) posGuidSell=res7.Position.Id;}
	
   if (posGuidBuy2!=Guid.Empty && Trade.GetPosition(posGuidBuy2).Pips>SL1) 	
	{var res8 = Trade.UpdateMarketPosition(posGuidBuy2,Trade.GetPosition(posGuidBuy2).OpenPrice+Instrument.Spread+dlt,Trade.GetPosition(posGuidBuy2).TakeProfit, null); 
		...
		Print("frU={0}-kfu={1} frD={2}-kfd={3}",_frInd.TopSeries[Bars.Range.To-i],kfu,_frInd.BottomSeries[Bars.Range.To-i],kfd);
			  ...
			  if(Bars[Bars.Range.To-i].Low<zmin)  { zmin=Bars[Bars.Range.To-i].Low;  tmin=Bars[Bars.Range.To-i].Time;  }
			  	  ...
			  	  i++;
					 ...
					 Print("Init OK! ---- {0} frU1={1} frU2={2} frU3={3} frD1={4} frD2={5} frD3={6}  zmax={7} zmin={8}",
			  Bars[Bars.Range.To-i].Time,frU1,frU2,frU3,frD1,frD2,frD3,zmax,zmin);
		} 
//===============  END History ===============================================================================
//===============  Read conf   ===============================================================================		
						 ...
						 {    
							...
							{ 	k2++; 
								...
								{	
									...
									if(k2==7) NKZ=Convert.ToInt32(st);
									if(k2==8) TP2=Convert.ToInt32(st);
									if(k2==9) lot=Convert.ToDouble(st);
									if(k2==10) SL1=Convert.ToInt32(st);
									if(k2==11) dl=Convert.ToInt32(st);
									if(k2==12) frac=Convert.ToInt32(st);
									if(k2==13) kof=Convert.ToInt32(st);
								}
								st="";
							} else { st = st+line[j]; }	
						  }
        				}
		         Print("File INIT name={0} - kl={1} tu={2} td={3} nu={4} nd={5} n2={6} NKZ={7} TP2={8} lot={9} SL1={10} dl={11} frac={12} kof={13}",name,kl,tu,td,nu,nd,n2,NKZ,TP2,lot,SL1,dl,frac,kof);
			 }  			
		}  
// END InitFile()  ===============================================================================
