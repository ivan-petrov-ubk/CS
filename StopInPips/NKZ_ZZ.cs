...
//======================================================================================================================================
			...
			{  
				...
				if(zz3<zz2 && zz2>zz1)  
				{ // ВВЕРХУ
					Print("{2}-ZZUP - {0} - {1}",Bars[Bars.Range.To-1].Time,zz2,Instrument.Name);
					if(posGuidBuy!=Guid.Empty && Trade.GetPosition(posGuidBuy).State==PositionState.Pending){
						Print("{4}-BuyUpdate {0} - Price={1} - SL={2} - TP={3}",Bars[Bars.Range.To-1].Time,zz2-(NKZ*Instrument.Point)+Instrument.Spread,zz2-((NKZ+SL)*Instrument.Point),zz2-((NKZ-TP)*Instrument.Point),Instrument.Name);
					var result3 = Trade.UpdatePendingPosition(posGuidBuy, 0.1, 
						...
						Print("{4}-BuyOpen {0} - Price={1}",Bars[Bars.Range.To-1].Time,zz2-(NKZ*Instrument.Point)+Instrument.Spread);
						 ...
						 var result = Trade.OpenPendingPosition(Instrument.Id, ExecutionRule.BuyLimit, 0.1, 
								             ...
								             Stops.InPips(SL,TP), null, null, null);	
				...
				if(zz3>zz2 && zz2<zz1)  
				{ // ВНИЗУ
				     Print("{2}-ZZDOWN - {0} - {1}",Bars[Bars.Range.To-1].Time,zz2,Instrument.Name);
					 if(posGuidSell!=Guid.Empty && Trade.GetPosition(posGuidSell).State==PositionState.Pending){
						Print("{4}-SellUpdate {0} - Price={1} - SL={2} - TP={3}",Bars[Bars.Range.To-1].Time,zz2+(NKZ*Instrument.Point)-Instrument.Spread,zz2+((NKZ+SL)*Instrument.Point)+Instrument.Spread,zz2+((NKZ-TP)*Instrument.Point),Instrument.Name);
					 ...
					 if(posGuidSell==Guid.Empty){
						Print("{4}-SellOpen {0} - Price={1}",Bars[Bars.Range.To-1].Time,zz2+(NKZ*Instrument.Point)+Instrument.Spread);
