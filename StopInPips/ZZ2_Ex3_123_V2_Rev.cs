...
{
    [TradeSystem("ZZ2_Ex3_123_V2_Rev")]      //copy of "ZZ2_Ex3_123_V2"
    public class ZZ_Ex1 : TradeSystem
    {
		[Parameter("ExtDepth3:", DefaultValue = 3)]
        public int ED3 { get; set; }	
		[Parameter("ExtDepth50:", DefaultValue = 35)]
...
//====== ВВЕРХУ ПИК =====================================================================================================================
			...
			{var res = Trade.CloseMarketPosition(posGuidBuy); if (res.IsSuccessful) posGuidBuy = Guid.Empty;}
					 ...
					 if(torgD) // ВВЕРХУ
							{
							if(torg && posGuidSell==Guid.Empty){ torgD=false;
								var result2 = Trade.OpenMarketPosition(Instrument.Id, ExecutionRule.Buy, 0.1,Instrument.Ask, -1,Stops.InPips(SL,TP), null, null);
...
//==== ВНИЗУ ПИК ======================================================================================================================				
			...
			{var res = Trade.CloseMarketPosition(posGuidSell); if (res.IsSuccessful) posGuidSell = Guid.Empty;}
					...
					if(torgU)//  ВНИЗУ
						{   
							if(torg && posGuidBuy==Guid.Empty){ torgU=false;
								var result3 = Trade.OpenMarketPosition(Instrument.Id, ExecutionRule.Sell, 0.1,Instrument.Bid, -1, Stops.InPips(SL,TP), null, null);
