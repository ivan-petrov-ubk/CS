...
//  FRACTALS ================================================================================================================
			...
			{  if (Trade.GetPosition(posGuidSell).State==PositionState.Active) 
				   Trade.UpdateMarketPosition(posGuidSell, Bars[Bars.Range.To-5].High, null, null); }
			
			if(_frInd.BottomSeries[Bars.Range.To-5]>0) 
			{  if (Trade.GetPosition(posGuidBuy).State==PositionState.Active) 
				   Trade.UpdateMarketPosition(posGuidBuy, Bars[Bars.Range.To-5].Low, null, null); }			
//  ОБРАБОТКА ZigZag ========================================================================================================
			_wprInd.ReInit();
				  ...
				  if(x<(Bars[zzd1].High-Bars[zzd2].Low))   { 
					  ...
					  Print("SELL {0} = {1}+{2} -- {3}",cnO,Bars[zzd1].High,cnU,Bars[zzi1].Time);	
						if(torg && posGuidSell==Guid.Empty) {
			   				var result1 = Trade.OpenPendingPosition(Instrument.Id, ExecutionRule.SellLimit, 0.1,  cnO, 0, Stops.InPips(SL,null), null, null, null);
				...
				{  Print("Внизу Максимум");
					...
					{   cnO=Math.Round(Bars[zzd1].Low+cnD-Instrument.Spread,5);
						...
						if(torg && posGuidBuy==Guid.Empty) 
					     { var result = Trade.OpenPendingPosition(Instrument.Id, ExecutionRule.BuyLimit, 0.1,  cnO, 0, Stops.InPips(SL,null), null, null, null);
					   ...
					   if (torg && posGuidBuy!=Guid.Empty && Trade.GetPosition(posGuidBuy).State==PositionState.Pending)
						 { var res = Trade.CancelPendingPosition(posGuidBuy);
									...
									var result1 = Trade.OpenPendingPosition(Instrument.Id, ExecutionRule.BuyLimit, 0.1,  cnO, 0, Stops.InPips(SL,null), null, null, null);
