...
{
    [TradeSystem("ZZ2_Ex3_123_V2_1")]      //copy of "ZZ2_Ex3_123_V2"
			...
			{var res = Trade.CloseMarketPosition(posGuidBuy); if (res.IsSuccessful) posGuidBuy = Guid.Empty;}
					 ...
					 // ВВЕРХУ
							...
							if(torgD && torg && posGuidSell==Guid.Empty){ torgD=false;
								var result2 = Trade.OpenMarketPosition(Instrument.Id, ExecutionRule.Sell, 0.1,Instrument.Ask, -1,Stops.InPips(SL,null), null, null);
			...
			{var res = Trade.CloseMarketPosition(posGuidSell); if (res.IsSuccessful) posGuidSell = Guid.Empty;}
				...
				//  ВНИЗУ
							...
							if(torgU && torg && posGuidBuy==Guid.Empty){ torgU=false;
								var result3 = Trade.OpenMarketPosition(Instrument.Id, ExecutionRule.Buy, 0.1,Instrument.Bid, -1, Stops.InPips(SL,null), null, null);
